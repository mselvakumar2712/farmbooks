...
/** Crop Starts **/
    ...
    function post_crop(){ 
        $crop_details = array(	
			...
			'seedling_date'			=> $this->input->post('seed_date'),
            'is_direct_seeding'     => ($this->input->post('direct_seed')) ? $this->input->post('direct_seed'):0,
            'transplant_date'       => ($this->input->post('direct_seed') == 0) ? $this->input->post('transplant_date'):"",
    ...
    function cropByID($crop_id){
		$this->db->select('trv_crop.*, trv_crop_variety_master.variety AS variety_name, trv_crop_name_master.name AS crop_name, trv_crop_sub_category_master.name AS subcategory_name, trv_crop_category_master.name AS category_name, trv_farmer.profile_name');
        $this->db->where(array('trv_crop.id' => $crop_id, 'trv_crop.status' => 1));
        $this->db->from('trv_crop');
		$this->db->join('trv_crop_variety_master', 'trv_crop_variety_master.id = trv_crop.variety_id');
        $this->db->join('trv_crop_name_master', 'trv_crop_name_master.id = trv_crop.crop_id');
        $this->db->join('trv_crop_sub_category_master', 'trv_crop_sub_category_master.id = trv_crop.subcategory_id');
    ...
    function updatecrop($crop_id){
		...
		$cost_expense_details = array(	
            'crop_id'        => $this->input->post('variety'),
            'seed_cost'      => $this->input->post('seed_cost'),
            'farmer_id'      => $this->input->post('farmer_id')
        );
		return $this->db->update('trv_update_expense', $cost_expense_details, array('farmer_id' => $this->input->post('farmer_id'), 'crop_id' => $this->input->post('variety')));
...
/** Update Crop related API functions **/
    ...
    function add_updatecrop(){
        ...
        if($this->input->post('update_type') == 1) {            
            ...
            $brand_name     = $brand_nameData;
            $name			= "";
            $weed_type      = $this->input->post('type_weeding');
            $man_days       = ($this->input->post('weeding_no_of_man'))?$this->input->post('weeding_no_of_man'):"";
            $machine_hours  = ($this->input->post('weeding_machine_hrs'))?$this->input->post('weeding_machine_hrs'):"";
		...
		if(!empty($getrow)){
			if($this->input->post('update_type') == 1){ 
				$nutrient_cost = /*$getrow['nutrient_cost']+*/$this->input->post('cost_of_nutrient');
				...
				$fertilizer_cost = /*$getrow['fertilizer_cost']+*/$this->input->post('cost_of_fertilizer');
				$pesticide_cost = $getrow['pesticide_cost'];
				$weeding_cost = $getrow['weeding_cost'];
			} else if($this->input->post('update_type') == 3){
				$nutrient_cost = $getrow['nutrient_cost'];
				$fertilizer_cost = $getrow['fertilizer_cost'];
				$pesticide_cost = /*$getrow['pesticide_cost']+*/$this->input->post('cost_of_pesticide');
				$weeding_cost = $getrow['weeding_cost'];
			} else {
				$nutrient_cost = $getrow['nutrient_cost'];
				$fertilizer_cost = $getrow['fertilizer_cost'];
				$pesticide_cost = $getrow['pesticide_cost'];
				$weeding_cost = /*$getrow['weeding_cost']+*/$this->input->post('cost_of_weeding');
			}
           ...
           return $this->db->update('trv_update_expense', $cost_expense_details, array('id' => $getrow['id']));
        } else {
			$cost_expense_details = array(	
				'crop_id'               => $variety_id,  
				'farmer_id'             => $this->input->post('farmer_id'),
          ...
          return $this->db->insert('trv_update_expense', $cost_expense_details);
        }		
    }
    ...
    function upateCropByCropID($updatecrop_id){
		$this->db->select('trv_update_crop.*, trv_farmer.profile_name AS profile_name, trv_crop_variety_master.variety AS variety_name, trv_brand_master.name AS brandname, trv_uom_master.full_name AS fullname');
		$this->db->select('trv_nutrient_master.name AS nutrient_name, trv_fertilizer_master.name AS fertilizer_name, trv_pesticide_master.name AS pesticide_name');
        $this->db->where(array('trv_update_crop.id' => $updatecrop_id, 'trv_update_crop.status' => 1));
        $this->db->from('trv_update_crop');
		$this->db->join('trv_farmer', 'trv_farmer.id = trv_update_crop.farmer_id');
		$this->db->join('trv_crop_variety_master', 'trv_crop_variety_master.id = trv_update_crop.variety_id');
		$this->db->join('trv_brand_master', 'trv_brand_master.id = trv_update_crop.brand_name', 'left');
		$this->db->join('trv_uom_master', 'trv_uom_master.id = trv_update_crop.qty_uom');
		$this->db->join('trv_nutrient_master', 'trv_nutrient_master.id = trv_update_crop.name', 'left');
		$this->db->join('trv_fertilizer_master', 'trv_fertilizer_master.id = trv_update_crop.name', 'left');
		$this->db->join('trv_pesticide_master', 'trv_pesticide_master.id = trv_update_crop.name', 'left');
    ...
    function update_crop($updatecrop_id){
        ...
        if($this->input->post('update_type') == 1){            
            ...
            $variety_id     = $this->input->post('weeding_variety');
            $process_date   = $this->input->post('weeding_dosage_date');
            $dosage         = ($this->input->post('weeding_dosage'))?$this->input->post('weeding_dosage'):0;
            $qty            = $this->input->post('weeding_quantity');
			$qty_uom        = $this->input->post('weeding_quantity_uom');
            $brand_name     = $brand_nameData;
            $name			= "";
...
/** Crop harvest related API functions **/
    function cropHarvestList(){        
        $this->db->select('trv_crop_harvest.*, trv_crop_variety_master.variety AS variety_name,trv_farmer.profile_name AS profile_name, trv_uom_master.full_name AS fullname');
        $this->db->where(array('trv_crop_harvest.status' => 1));
        $this->db->from('trv_crop_harvest');
        $this->db->join('trv_crop_variety_master', 'trv_crop_variety_master.id = trv_crop_harvest.variety_id');
		$this->db->join('trv_farmer', 'trv_farmer.id = trv_crop_harvest.farmer_id');
		$this->db->join('trv_uom_master', 'trv_uom_master.id = trv_crop_harvest.sales_qty_uom', 'left');
        return $this->db->get()->result();
    }
    ...
    function cropharvest(){
		...
		for($i=0;$i<count($output_product);$i++){
			$crop_details = array( 
				...
				'sales_available'   => ($this->input->post('sales_available'))?$this->input->post('sales_available'):0,
    ...
    function editcropharvest($crop_id){
		...
		$this->db->select('trv_crop_harvest.*, GROUP_CONCAT(trv_crop_harvest.id SEPARATOR ",") as harvestid');
		$this->db->select('trv_crop_variety_master.variety AS variety_name, trv_farmer.profile_name AS profile_name, trv_uom_master.full_name AS fullname');
        $this->db->where(array('trv_crop_harvest.id' => $crop_id, 'trv_crop_harvest.status' => 1));
        $this->db->from('trv_crop_harvest');
        $this->db->join('trv_crop_variety_master', 'trv_crop_variety_master.id = trv_crop_harvest.variety_id');
		$this->db->join('trv_farmer', 'trv_farmer.id = trv_crop_harvest.farmer_id');
		$this->db->join('trv_uom_master', 'trv_uom_master.id = trv_crop_harvest.sales_qty_uom', 'left');
	...
	function editpostharvest($postharvest_id){
		...
		$this->db->select('trv_crop_variety_master.variety AS variety_name, trv_farmer.profile_name AS profile_name');
        $this->db->where(array('trv_post_harvest.id' => $postharvest_id, 'trv_post_harvest.status' => 1));
    ...
    function updatecropharvest($crop_id){ 
        ...
        for($i=0;$i<count($output_product);$i++){
		    ...
		    $this->db->update('trv_crop_harvest', $crop_details, array('id' => $crop_id));
			...
			return $this->db->update('trv_update_expense', $cost_expense_details, array('farmer_id' => $this->input->post('farmer_id'), 'crop_id' => $this->input->post('crop_variety')));
...
/** Post Crop harvest related API functions **/
    function postHarvestList() { 
		...
		$this->db->group_by('trv_post_harvest.crop_id');
        return $this->db->get()->result();	
    }
	
	function postHarvestOutputList($farmer_id, $cropId){ 
		$this->db->select('trv_post_harvest.id, trv_post_harvest.output_product, trv_post_harvest.output_qty, trv_post_harvest.output_qty_uom');
		$this->db->select('trv_post_harvest.sale_available, trv_post_harvest.qty_for_sale, trv_post_harvest.sales_qty_uom');
        $this->db->where(array('trv_post_harvest.farmer_id' => $farmer_id, 'trv_post_harvest.crop_id' => $cropId, 'trv_post_harvest.status' => 1));
        $this->db->from('trv_post_harvest');
        return $this->db->get()->result();	
    }
    ...
    function postharvest(){ 
        ...
        for($i=0;$i<count($output_product);$i++){   
			$crop_details = array(
				...
				'qty_for_sale'         => ($quantity_uom)? $quantity_uom[$i]:"",
				'sales_qty_uom'        => ($sales_qty_uom)? $sales_qty_uom[$i]:"",
				'status'               => 1,
				'updated_on'           => date('Y-m-d H:i:s'),
				'updated_by'           => $this->session->userdata('user_id')
			);					  
			...
			$this->db->insert('trv_post_harvest_master', $crop_master_details); 
		} 		
    ...
    function updatepostharvest($crop_id){  
        ...
        for($i=0;$i<count($output_product);$i++){			 
			$crop_details = array(
				...
				'post_harvesting_cost' => $this->input->post('cost_post_harvesting'),
				'sale_available'       => $sales_qty,
				'qty_for_sale'         => $quantity_uom[$i],
				'sales_qty_uom'        => $sales_qty_uom[$i],
				'status'               => 1,
				'updated_on'           => date('Y-m-d H:i:s'),
				'updated_by'           => $this->session->userdata('user_id')
			);				 
			if(isset($output_id[$i]) && $output_id[$i]){
				$this->db->update('trv_post_harvest', $crop_details, array('id' => $output_id[$i]));    		
			} else {
				$this->db->insert('trv_post_harvest', $crop_details);	
				...
				$this->db->insert('trv_post_harvest_master', $crop_details_list); 
			}
		}
		$cost_expense_details = array(	
            'crop_id'               => $this->input->post('variety'),
