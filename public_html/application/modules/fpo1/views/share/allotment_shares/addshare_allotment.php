...
$CI = get_instance();
$current_avaialable_share = $total_avaialable_share;
?>
                    ...
                    <div class="card">
                        <div class="card-body">
						    <div class="container-fluid">
								<form action="<?php echo base_url('fpo/share/postShareAllotment'); ?>" name="sentMessage" id="allotmentForm" novalidate="novalidate" method="post">                                   
										...
										<table class="table table-bordered">
											...
											<tbody>
                                                <?php if(isset($share_application)) {
													$folioCount = ($allottedFolioCount)?$allottedFolioCount:0;
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-right">
														...
														if($allottedFolioNo != null && $allottedFolioNo->folio_number) {
															...
															//$allottedFolioCount = $CI->Share_Model->getShareHoldersFolioCount($this->session->userdata('user_id'), $share_application[$i]->holder_id, $share_application[$i]->member_type);
															echo str_pad(($folioCount+1), 4, '0', STR_PAD_LEFT);
                                    ...
                                    <?php if(isset($share_application)){
											$actualFolioCount = ($allottedFolioCount)?$allottedFolioCount:0;
                                          for($i=0;$i<count($share_application);$i++){ 
                                            if($share_application[$i]->member_type == 1){
											...
											if($share_application[$i]->no_of_share > $total_avaialable_share){
												$avaialable_share = $total_avaialable_share;
												$total_avaialable_share = 0;
											} else {
												$total_avaialable_share = $total_avaialable_share - $share_application[$i]->no_of_share;
												$avaialable_share = $share_application[$i]->no_of_share;
											}
										?>
									<div class="row row-space">
										<div class="form-group col-md-4">
										...
										<div class="form-group col-md-4">											
											...
											<?php } else { 
												//$allottedFolioCount = $CI->Share_Model->getShareHoldersFolioCount($this->session->userdata('user_id'), $share_application[$i]->holder_id, $share_application[$i]->member_type);
												//echo $actualFolioCount;
												$actualFolioCount = str_pad(($actualFolioCount+1), 4, '0', STR_PAD_LEFT);												
											?>
												<input type="text" class="form-control numberOnly" maxlength="4" id="original_folio_number" name="<?php echo "share_application[".$i."][folio_no]"; ?>" placeholder="Folio Number" required="required" data-validation-required-message="Enter folio number" value="<?php echo $actualFolioCount; ?>" readonly>
											<?php } ?>
 											<p class="help-block text-danger"></p>
										</div>
										<div class="form-group col-md-2">
											<label for="inputAddress2">Shares Applied <span class="text-danger">*</span></label>	
											<input type="text" class="form-control numberOnly" maxlength="4" id="original_shares_applied" name="<?php echo "share_application[".$i."][share_applied]"; ?>" placeholder="Shares Applied" required="required" data-validation-required-message="Enter shares applied" value="<?php echo $share_application[$i]->no_of_share; ?>" readonly>
											<p class="help-block text-danger"></p>
										</div>
										<div class="form-group col-md-2">
											<label for="inputAddress2">Shares Allotted <span class="text-danger">*</span></label>											
											<input type="text" class="form-control numberOnly <?php echo 'original_shares_alloted_'.$i; ?>" maxlength="3" id="original_shares_alloted" name="<?php echo "share_application[".$i."][share_alloted]"; ?>" placeholder="Shares Allotted" required="required" data-validation-required-message="Enter shares allotted" value="<?php echo $avaialable_share; ?>" onfocusout="verifyTotalAvailableShare(this.value, '<?php echo "original_shares_alloted_".$i; ?>', '<?php echo $share_application[$i]->no_of_share; ?>')">
											<p class="help-block text-danger"></p>
										</div>
									</div>
                                    <?php $actualFolioCount = $actualFolioCount; }} ?>
									</div>
									...
									<div class="row row-space" id="buttonContent" style="">
								        ...
								        <a href="<?php echo base_url('fpo/share/shareallotment'); ?>" class="btn-fs btn btn-outline-dark mt-10"><i class="fa fa-close" aria-hidden="true"></i> Cancel</a>		  
									   </div>				
								   </div> 
...
function verifyTotalAvailableShare(shareAlloted, shareAllotedHolder, shareApplied){
	if(shareAlloted <= shareApplied){
		...
		swal("Sorry!", "Allotted share should not exceeds the applied share", "warning");
		$('.'+shareAllotedHolder).val('');
		$('.'+shareAllotedHolder).focus();
	}
}
