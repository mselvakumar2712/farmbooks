										...
										<table class="table table-bordered">
											...
											<tbody>																																			
												...
												<tr id="row_p_and_l" bgcolor="#A9A9A9" onclick="getDefaultByAccountCode(this.id, 1, 1, 'row_p_and_l_1')">
													...
													<?php if(substr($netProfit, 0, 1) != '-'){ ?>
													<td align="right"><?php echo number_format(ltrim($netProfit, '-'), 2); //echo (substr($netProfit, 0, 1) == '-')?' Dr':' Cr'; ?></td>
													<td></td>
													<?php //$creditValue = $creditValue + $netProfit; 
													} else { ?>
													<td></td>
													<td align="right"><?php echo number_format(ltrim($netProfit, '-'), 2); //echo (substr($netProfit, 0, 1) == '-')?' Dr':' Cr'; ?></td>
													<?php //$debitValue = ($debitValue) - ($netProfit); 
													} ?>
												</tr>	
                                    ...
                                    <td align="right" class="text-danger"><strong><?php //echo number_format(($liablityCost)?ltrim($liablityCost, '-'):0, 2);echo (substr($liablityCost, 0, 1) == '-')?' Dr':' Cr'; ?></strong></td>
											...
											<tbody>
												...
												<tr id="row_closing" bgcolor="#A9A9A9">
												   ...
												   <?php if(substr($openingStock, 0, 1) != '-'){ ?>
																<td align="right"><?php echo number_format(ltrim($openingStock, '-'), 2);//echo (substr($openingStock, 0, 1) == '-')?' Dr':' Cr'; ?></td>
												   <td></td>
												  <?php $creditValue = $creditValue+$openingStock; } else { ?>
												   <td></td>
												   <td align="right"><?php echo number_format(ltrim($openingStock, '-'), 2);//echo (substr($openingStock, 0, 1) == '-')?' Dr':' Cr'; ?></td>
												   <?php $debitValue = ($debitValue) + ($openingStock); } ?>
												</tr>	
												<?php } ?>
												<?php 
...
/* $('#search_trail_ledger').click(function() {
    ...
    $.ajax({
        url:"<?php echo base_url();?>fpo/finance/postTrialBalance",
		type:"POST",
		data:trialEntry,
		dataType:"html",
        cache:false,			
