...
<!--									<a href="#" id="" class="del btn btn-danger">Delete</a>-->
									<a href="<?php echo base_url('fpo/farmer/landlist');?>" id="ok" class="btn btn-fs btn-outline-dark"><i class="fa fa-arrow-circle-left" aria-hidden="true"></i> Back</a>	
									<a href="<?php echo base_url('fpo/farmer/landlist');?>" id="cancel" class="btn btn-fs btn-outline-dark" style="display:none;"><i class="fa fa-close" aria-hidden="true"></i> Cancel</a>							
								</div>
						       </form>
						     </div>
					      </div>
...
//                  $("#both_organic_certifiation").prop('required',true);
			...
			$('a.del').click(function() {
                ...
                swal({
                 ...
                 if (result.value) {          
                    ...
                    $.ajax({
                      url: "<?php echo base_url();?>fpo/farmer/deleteLandDetail/"+land_id,
                      type: "POST",
                      data: {
                         this_delete: land_id,
                      },
                      ...
                      success: function() {        
                         setTimeout(function() {
                          window.location.replace("<?php echo base_url('fpo/farmer/landlist')?>");
                         }, 1000);
                      },
                      error: function() {
    ...
    //jQuery method
      $.ajax({
        url:"<?php echo base_url();?>fpo/farmer/editland/"+land_id,
        type:"GET",
        dataType:"html",
        cache:false,      
        success:function(response){      
            ...
            if(responseArray.status == 1){                                                
                ...
                if(responseArray.land_list[0].land_holdings==1){
                    ...
                    var href_value="<?php echo base_url('fpo/farmer/addland?id=');?>"+responseArray.land_list[0].farmer_id;
...
//                                            land_measure_unit[j].value = land_identification[j].measurement_unit;          
                                    ...
                                    if(responseArray.land_list[0].organic_former != null) { 
									document.getElementById("organic_former").value=responseArray.land_list[0].organic_former;    
                                    }
...
$('form').submit(function(e){
        ...
        $.ajax({
            url:"<?php echo base_url();?>fpo/farmer/updateland/"+land_id,
            type:"POST",
            data:farmerdata,
            dataType:"html",
            cache:false,      
            success:function(response){      
              ...
              //console.log(response);
                if(responseArray.status == 1) {
                     ...
                     setTimeout(function(){ 
                       window.location = "<?php echo base_url('fpo/farmer/landlist')?>";
                     }, 500);					
				} else {
...
function getFarmerProfileLocation(type){
	...
	if(type == 1){
		...
		var url = "<?php echo base_url();?>fpo/farmer/edit_Farmer/"+farmer_id;
	} else {
		var land_id = '<?php echo $land_id; ?>';
		var url = "<?php echo base_url();?>fpo/farmer/editland/"+land_id;
	}
