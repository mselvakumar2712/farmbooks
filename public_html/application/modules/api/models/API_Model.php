...
class API_Model extends CI_Model {
    ...
    /* 
		To get the city by by block code start from here
		$citycode = 2901 (Its based on the pincode send by the user)
		$is_local = 1 (means English and if it is 2 means the local language)
	*/	
	function getCityByBlock($citycode,$is_local) {
        if ($is_local == 1)
		{
		...
		{
			$this->db->select('trv_district_master.district_code, trv_district_master.state_code, trv_district_master.name_local name');
			$this->db->where(array('district_code' => $citycode, 'status' => '1') );
			$this->db->order_by("district_code", "desc");
			$this->db->from('trv_district_master');
		}
    ...
    function getblockbydistrict( $district_code,$is_local ) {
        if ($is_local == 1)
		{
		...
		{
			$this->db->select('trv_block_master.block_code, trv_block_master.district_code, trv_block_master.name_local name');
            $this->db->where( array('status' => '1') );
            $this->db->where_in('trv_block_master.district_code', $district_code);
            $this->db->order_by("trv_block_master.block_code", "desc");
            $this->db->from('trv_block_master');
	...
	/* 
		To get the city by by block code start from here
		$state_id = 29 (Its based on the statecode from the pincode send by the user)
		$is_local = 1 (means English and if it is 2 means the local language)
	*/
    function getStateByCity($state_id,$is_local) {
		if ($is_local == 1)
		{
		...
		{
			$this->db->select('trv_state_master.state_code, trv_state_master.name_local name');
			$this->db->where(array('state_code' => $state_id, 'status' => '1') );
			$this->db->order_by("state_code", "desc");
			$this->db->from('trv_state_master');
		}
	...
	function getPanchayatByBlockcode( $block_code,$is_local ) {
     if ($is_local == 1)
		{
		...
		{
			 $this->db->select('id, panchayat_code, name_local name, status');
			 $this->db->where(array('block_code' => $block_code, 'status' => '1') );
			 $this->db->order_by("name", "asc");
			 $this->db->from('trv_panchayat_master');
		}
    ...
    function getVillageByPanchayatcode($panchayat_id, $is_local ) {
        if ($is_local == 1)
		{
		...
		{
            $this->db->select('trv_village_master.id, trv_village_master.panchayat_code, trv_village_master.name_local name, trv_village_master.pincode, trv_village_master.status');
            $this->db->where(array('panchayat_code' => $panchayat_id, 'status' => '1') );
            $this->db->order_by("id", "desc");
            $this->db->from('trv_village_master');
		}
    ...
    function getdealerinfo( $state, $district, $taluk,$dealer_type ) {
		...
		$this->db->where(array('dealer_type' => $dealer_type));
	...
	function getTalukByDistrictCode( $district_code, $is_local ) {
        if ($is_local == 1)
		{
		...
		{
            $this->db->select('trv_taluk_master.id, trv_taluk_master.district_code, trv_taluk_master.taluk_code, trv_taluk_master.name_local name');
            $this->db->where( array('status' => '1') );  
            $this->db->where_in('trv_taluk_master.district_code', $district_code);
            $this->db->order_by("trv_taluk_master.id");
            $this->db->from('trv_taluk_master');    
	...
	function getcropcategoryList($is_local) {
        if ($is_local == 1)
		{
            $this->db->select('trv_crop_category_master.id, trv_crop_category_master.name, trv_crop_category_master.status');
            $this->db->where(array('status' => '1'));
            $this->db->order_by("id");
		...
		{
            $this->db->select('trv_crop_category_master.id, trv_crop_category_master.name_local name, trv_crop_category_master.status');
            $this->db->where(array('status' => '1'));
            $this->db->order_by("id");
            $this->db->from('trv_crop_category_master');
		}
	...
	function getcropsubcategory($category_id,$is_local) {
        if ($is_local == 1)
		{
            $this->db->select('id, crop_category_id, name, status');
            $this->db->where(array('crop_category_id' => $category_id, 'status' => '1'));
            $this->db->from('trv_crop_sub_category_master');
		}
		else
		{
            $this->db->select('id, crop_category_id, name_local name, status');
            $this->db->where(array('crop_category_id' => $category_id, 'status' => '1'));
            $this->db->from('trv_crop_sub_category_master');
		}
        return $this->db->get()->result();
	...
	function getcropname($subcategory_id,$is_local) {
		if ($is_local == 1)
		{
            $this->db->select('id, sub_category_id, name, status');
            $this->db->where(array('sub_category_id' => $subcategory_id, 'status' => '1'));
            $this->db->from('trv_crop_name_master');
		}
		else
		{
            $this->db->select('id, sub_category_id, name_local name, status');
            $this->db->where(array('sub_category_id' => $subcategory_id, 'status' => '1'));
            $this->db->from('trv_crop_name_master');
		}
        return $this->db->get()->result();
	...
	function getcropvariety($name_id,$is_local) {
		if ($is_local == 1)
		{
            $this->db->select('id, name_id, variety, status');
            $this->db->where(array('name_id' => $name_id, 'status' => '1'));
            $this->db->from('trv_crop_variety_master');
		}
		else
		{
            $this->db->select('id, name_id, variety_local variety, status');
            $this->db->where(array('name_id' => $name_id, 'status' => '1'));
            $this->db->from('trv_crop_variety_master');
		}
        return $this->db->get()->result();
	...
	function getfarmimplementsList($is_local) {
        if ($is_local == 1)
		{
		...
		{
            $this->db->select('id, Primary_Yes, Name_local Name, status');
            $this->db->where(array('status' => '1'));
            $this->db->order_by("id");
            $this->db->from('trv_farm_implementation');   
		}
	...
	function getfarmimplementsmakeList($implements_id,$is_local) {
		if ($is_local == 1)
		{
            $this->db->select('Make');
		...
		{
            $this->db->select('make_local Make');
            $this->db->where(array('implements_id' => $implements_id, 'status' => '1'));
            $this->db->order_by("Make");
            $this->db->from('trv_farm_implementation_make_model');
            $this->db->distinct();   
	...
	function getfarmimplementsmodelList($makename,$is_local) {
		if ($is_local == 1)
		{
		...
		{
            $this->db->select('id, Model_local Model');
            $this->db->where(array('Make_local' => $makename, 'status' => '1'));
            $this->db->order_by("id");
            $this->db->from('trv_farm_implementation_make_model');   
		}
        return $this->db->get()->result();	
	...
	function getlivestocksList($is_local) {
		if ($is_local == 1)
		{
		...
		{
            $this->db->select('id, type, name_local name, status');
            $this->db->where(array('status' => '1'));
            $this->db->order_by("id");
            $this->db->from('trv_live_stocks_master');   
		}
	...
	function getlivestockvarietyList($live_stock_id,$is_local) {
        if ($is_local == 1)
		{
		...
		{
            $this->db->select('id, live_stock_id, variety_local variety, status');
            $this->db->where(array('live_stock_id' => $live_stock_id, 'status' => '1'));
            $this->db->order_by("id");
            $this->db->from('trv_live_stock_variety_master');  
		}
	...
	function getcertifiedproductList($is_local) {
        if ($is_local == 1)
		{
		...
		{
            $this->db->select('id, name_local name, status');
            $this->db->order_by("id");
            $this->db->from('trv_product_master');
		}
        return $this->db->get()->result();	
	...
	function getjurisdictionList($is_local) {
        if ($is_local == 1)
		{
            $this->db->select('id, name, status');
            $this->db->order_by("id");
    ...
    function getFarmerBankAccount($mobile_no) {         
        $this->db->select('`ref`.`id`, `ref`.`farmer_id`,tf.profile_name farmer_name, `ref`.`bank_id`, tbnm.name bank_name, `ref`.`branch_id`, tbm.branch_name, `ref`.`account_number`, `ref`.`ifsc_code`, `ref`.`mobile_no`');
        $this->db->from('fpo_farmer_bank_accounts as ref');
        $this->db->join('trv_bank_name_master tbnm','tbnm.id = ref.bank_id');
        $this->db->join('trv_bank_master tbm','tbm.id = ref.branch_id');
        $this->db->join('trv_farmer as tf', 'tf.id = ref.farmer_id');
        $this->db->where(array('ref.mobile_no' => $mobile_no, 'ref.status' => '1'));
    	  return $this->db->get()->result();
   }
  
   function addFarmerBankAccount() {                                 
      $post_data = json_decode(file_get_contents('php://input'), true);
      $farmer_bank_account = array(	
         'farmer_id'             => $post_data['farmer_id'],
      ...
      return $this->db->insert('fpo_farmer_bank_accounts', $farmer_bank_account);
   }

   function updateFarmerBankAccount() {                                 
      $post_data = json_decode(file_get_contents('php://input'), true);
      if($post_data['id'] >0) {
            $farmer_bank_account = array(	
               'farmer_id'             => $post_data['farmer_id'],
            ...
            return $this->db->update('fpo_farmer_bank_accounts', $farmer_bank_account, array('id' => $post_data['id']));
      }
      else {
         return false;
      }
