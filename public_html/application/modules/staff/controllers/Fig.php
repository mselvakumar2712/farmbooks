...
class Fig extends CI_Controller {
   ...
   public function __construct() {
      ...
      header('Access-Control-Allow-Origin: *');
		if(!check_staff_permission($_SESSION['profile_type'], 104) || !check_staff_permission($_SESSION['profile_type'], 105)){
...
/** View FIG Representative profile related view by using the given functions **/
	public function figrepresentativelist(){
		if(!check_staff_permission($_SESSION['profile_type'], 10501)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
	...
	public function addfigrepresent(){
		if(!check_staff_permission($_SESSION['profile_type'], 10502)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
        $data['page'] = 'FIG Representative';
...
/** End FIG Representative views **/  
    ...
    public function postfigrepresent() {        
		...
		if($this->Fig_Model->addfigrepresent()) {  
			...
			redirect('staff/fig/figrepresentativelist');
		} else {
...
/** View FIG profile related view by using the given functions **/
    public function index(){
		if (!check_staff_permission($_SESSION['profile_type'], 10401)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
	...
	public function addfig(){
		if (!check_staff_permission($_SESSION['profile_type'], 10402)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
