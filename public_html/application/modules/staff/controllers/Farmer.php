...
class Farmer extends CI_Controller {
    ...
    public function __construct() {
		...
		if(!check_staff_permission($_SESSION['profile_type'], 106)){
...
/** View Farmers profile related view by using the given functions **/
    public function profilelist(){
		if(!check_staff_permission($_SESSION['profile_type'], 10601)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
        $data['page'] = 'Farmer';
	...
	public function addfarmer(){
		if(!check_staff_permission($_SESSION['profile_type'], 10602)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
        $data['page'] = 'Farmer';
    ...
    public function editfarmer($farmer_id){
		if(!check_staff_permission($_SESSION['profile_type'], 10603)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
        $data['page'] = 'Farmer'; 
...
/** End Farmers views **/
    ...
    public function updateFarmer($farmer_id) {		
        if($this->Farmer_Model->updateFarmer($farmer_id)) {  
            ...
            redirect('staff/farmer/profilelist');
        } else {
            $this->session->set_flashdata('warning', 'Profile image is not uploaded');
            redirect('staff/farmer/editfarmer/'.$farmer_id);
        }        
        //echo json_encode($response);
	}
