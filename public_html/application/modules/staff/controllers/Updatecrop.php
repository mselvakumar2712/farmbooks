...
class Updatecrop extends CI_Controller {
   ...
   public function __construct() {
      ...
      header('Access-Control-Allow-Origin: *');
		if (!check_staff_permission($_SESSION['profile_type'], 10)){
...
/**Update crop views **/	
	public function index(){
		if(!check_staff_permission($_SESSION['profile_type'], 100201)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
	...
	public function add_updatecrop(){
		if(!check_staff_permission($_SESSION['profile_type'], 100202)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
           $data['page'] = 'Update Crop';
           ...
           $data['cropEntry_variety'] = $this->Crop_Model->cropvarietyDropdown();//print_r( $data['cropEntry_variety']);die;
	...
	public function edit_updatecrop(){
		if(!check_staff_permission($_SESSION['profile_type'], 100203)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
         $data['page'] = 'Crop Master';
         $data['page_title'] = 'View Update Crop';
...
/** Crop harvest views **/	
    public function cropharvest( $crop_id=null ){
		if(!check_staff_permission($_SESSION['profile_type'], 100301)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
    ...
    public function editcropharvest_value(){
		if(!check_staff_permission($_SESSION['profile_type'], 100303)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
        $data['page'] = 'Crop Harvest';
	...
	public function addharvest(){
		if(!check_staff_permission($_SESSION['profile_type'], 100302)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
         $data['page'] = 'Crop Harvest';
...
/** Post harvest views **/
	public function postharvest( $crop_id=null ){
		if(!check_staff_permission($_SESSION['profile_type'], 100401)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
    ...
    public function addpostharvest(){
		if(!check_staff_permission($_SESSION['profile_type'], 100402)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
...
/** Post Crop harvest end **/
   ...
   public function varietybycategory( $name_id ) {
	...
	public function expenseslist(){
		if(!check_staff_permission($_SESSION['profile_type'], 100501)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }		
        $data['page'] = 'Expenses';
	...
	public function addexpenses_update(){
		if(!check_staff_permission($_SESSION['profile_type'], 100502)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
        $data['page'] = 'Expenses';
	...
	public function editexpenses_update($expenses_id){
		if(!check_staff_permission($_SESSION['profile_type'], 100503)){
            redirect(base_url('staff/access_denied'), 'refresh');  
        }
         $data['page'] 		  = 'Expenses';
         $data['page_title']  = 'View Expenses Updation';
