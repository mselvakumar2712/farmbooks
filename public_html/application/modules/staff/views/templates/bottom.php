...
.modal-backdrop {
   ...
   <div class="icon-bar"> 
     <a href="#" class="chat float-left" data-toggle="modal" data-target="#myModal"><i class="fa fa-comments-o"></i></a> 
   </div>
   <!-- The Modal -->
...
<!-- chat icon ends-->
	  ...
	  <script>
         jQuery.ajax({
						url: '<?php echo base_url('staff/dashboard/supportEnquiry')?>',
						type: "GET",
						data:"",
     ...
     jQuery('#sendMessageSubmitButton').on('click', function(event){
                  ...
                  jQuery.ajax({
                     url:"<?php echo base_url();?>staff/dashboard/enquiry_information",
                     type:"POST",
                     data:messageData,
                     dataType:"html",
                     cache:false,			
            ...
            jQuery('a.signout').click(function() {
               swal({
                ...
                if (result.value) {          
                 ...
                 window.location.replace("<?php echo base_url('staff/signout')?>");
                }else {
