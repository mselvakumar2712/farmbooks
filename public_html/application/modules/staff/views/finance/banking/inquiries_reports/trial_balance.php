     ...
     <div class="container-fluid pl-0 pr-0">
       ...
       <div class="breadcrumbs">
                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            ...
                            <li><a class="active" href="<?php echo base_url('staff/finance/trialbalance');?>">Trial Balance	</a></li>
                        </ol>
                    </div>
                </div>
            </div>
		...
		<div class="content mt-3">
			...
			<form action="<?php echo base_url('staff/finance/postTrialBalance'); ?>" method="post" id="profit_loss_form" name="sentMessage" novalidate="novalidate" >
                  <div class="row">
						<div class="col-md-12">
							<div class="card">
								<div class="card-body">
									<div class="container-fluid">                                            
										<div class="row">
											<div class="form-group col-md-5">
												...
												<input class="form-control" type="date" id="trial_from" name="trial_from" value="<?php echo $fromValue; ?>">		
											</div>
											<div class="form-group col-md-5">
									...
									<div class="table-responsive mt-3" id="trial_account_balance"> 
										...
										<table class="table table-bordered">
											<tr bgcolor="#afd66b">		
												...
												<th class="text-center">Total (<i class="fa fa-inr" aria-hidden="true"></i>)</th>
											</tr>
											...
											<tbody>																																			
												...
												<tr id="row_p_and_l" bgcolor="#A9A9A9" onclick="getDefaultByAccountCode(this.id, 1, 1, 'row_p_and_l_1')">
													...
													<td align="right"><?php echo ltrim($netProfit, '-');echo (substr($netProfit, 0, 1) == '-')?' Dr':' Cr'; ?></td>
												</tr>	
											...
											<tr>
												...
												<td align="right" class="text-danger"><strong><?php echo ($liablityCost)?ltrim($liablityCost, '-'):"0.00";echo (substr($liablityCost, 0, 1) == '-')?' Dr':' Cr'; ?></strong></td>
											</tr>
											</tbody>
										</table>
										<?php //} ?>
										
										<?php //if(isset($assetGLData) && $assetGLData!=0){ ?>
										<table class="table table-bordered">
											<tr bgcolor="#afd66b">		
											...
											<tr>
												<td width="80%" class="text-right text-danger"><strong>Total (<i class="fa fa-inr" aria-hidden="true"></i>)</strong></td>												
												<td align="right" class="text-danger"><strong><?php echo ($assetCost)?ltrim($assetCost, '-'):"0.00";echo (substr($assetCost, 0, 1) == '-')?' Dr':' Cr'; ?></strong></td>
											</tr>
											</tbody>
										</table>
										<?php //} ?>
										
										<?php //if($incomeGLData && count($incomeGLData)!=0){ ?>
										<table class="table table-bordered">
											<tr bgcolor="#afd66b">		
											...
											<tbody>												
												...
												<!--<tr id="row_closing" bgcolor="#A9A9A9">
													...
													<td align="right"><?php //echo ltrim($closingStock, '-');echo (substr($closingStock, 0, 1) == '-')?' Dr':' Cr'; ?></td>
												</tr>-->	
												<?php //} ?>
											</tbody>
											...
											<tr>
												<td width="80%" class="text-right text-danger"><strong>Total (<i class="fa fa-inr" aria-hidden="true"></i>)</strong></td>												
												<td align="right" class="text-danger"><strong><?php echo ($incomeCost)?ltrim($incomeCost, '-'):"0.00";echo (substr($incomeCost, 0, 1) == '-')?' Dr':' Cr'; ?></strong></td>
											</tr>
											</tbody>
										</table>
										<?php //} ?>
										
										<?php //if($expenseGLData && count($expenseGLData)!=0){ ?>
										<table class="table table-bordered">
											<tr bgcolor="#afd66b">		
											...
											<tbody>
												...
												<tr id="row_closing" bgcolor="#A9A9A9">
													...
													<td align="right"><?php echo ltrim($openingStock, '-');echo (substr($openingStock, 0, 1) == '-')?' Dr':' Cr'; ?></td>
												</tr>	
												<?php } ?>
												<?php 
											...
											<tr>
												<td width="80%" class="text-right text-danger"><strong>Total (<i class="fa fa-inr" aria-hidden="true"></i>)</strong></td>												
												<td align="right" class="text-danger"><strong><?php echo ($expenseCost)?ltrim($expenseCost, '-'):"0.00"; echo (substr($expenseCost, 0, 1) == '-')?' Dr':' Cr'; ?></strong></td>
											</tr>
											</tbody>
										</table>
										<?php //} ?>
									...
									<table class="table table-bordered" id='ledgerShowTable'>
										...
										<tbody class=''>
											<tr bgcolor="#afd66b">		
												...
												<th class="text-center"><strong>Amount (<i class="fa fa-inr" aria-hidden="true"></i>)</strong></th>
											</tr>												
										</tbody>
...
function getChildrenNodeByAccountCode(selectedCode, value, has_child, toggleID, person_type_id, person_id) {	
	if(toggleID == 1){
		...
		if(has_child == 1){			
			...
			$.ajax({
				url:"<?php echo base_url();?>staff/finance/getChildNodeByCode",
				type:"POST",
				data:childList,
				dataType:"html",
				cache:false,			
					...
					if(responseArray.status == 1) {
						...
						$.each(responseArray.glCategory, function(key, value){
							if(value.has_child != 0){
								childNodeAppend += '<tr class="row_'+value.account_code2+'" id="'+value.account_code+'" onclick="getChildrenNodeByAccountCode(this.id, '+values+', '+value.has_child+', 1, '+value.person_type_id+')" bgcolor="'+bgColor+'">';
								childNodeAppend += '<td style="'+forceStyle+'"><strong>'+value.account_name+'</strong></td>';						
								if(value.amount.charAt(0) != '-'){
									childNodeAppend += '<td align="left">'+value.amount+' Cr</td>';
								} else {
									childNodeAppend += '<td align="left">'+value.amount.substr(1)+' Dr</td>';
								}
								childNodeAppend += '</tr>';
							} else {
								childNodeAppend += '<tr class="row_'+value.account_code2+'" id="'+value.account_code+'" onclick="getChildrenNodeByAccountCode(this.id, '+values+', '+value.has_child+', 1, '+value.person_type_id+')" bgcolor="'+bgColor+'">';
...
function childNodeAppendLists(parentAccountCode, person_type_id, person_id){
	...
	$.ajax({
        url:"<?php echo base_url();?>staff/finance/getAllChildNodeByCode",
        type:"POST",
        data:childList,
        dataType:"html",
        cache:false,			
				...
				$.each(responseArray.glCategory, function(key, value){
					if(value.has_child == 0){							
						...
						if(value.total_amount.charAt(0) != '-'){
							childNodeAppend += '<td align="right" width="15%">'+value.total_amount+' Cr</td>';
						} else {
							childNodeAppend += '<td align="right" width="15%">'+value.total_amount.substr(1)+' Dr</td>';
						}
						childNodeAppend += '</tr>';
					}
				}); 
...
/* $('#search_trail_ledger').click(function() {
    var trial_from = $('#trial_from').val();
    var trial_to = $('#trial_to').val();
    var trialEntry = {'trial_from':trial_from, 'trial_to':trial_to};
    console.log(trialEntry);
    
    $.ajax({
        url:"<?php echo base_url();?>staff/finance/postTrialBalance",
		type:"POST",
		data:trialEntry,
		dataType:"html",
        cache:false,			
		success:function(response) {
            ...
            response=response.trim();
            responseArray=$.parseJSON(response);
            var trialBalanceInquiry = '';
            if(responseArray.trialBalance.length < 1) {
               trialBalanceInquiry += '<tr><td colspan="10" class="text-center">No Records Found</td></tr>';
            }
            $.each(responseArray.trialBalance, function(key, value){                
                var type = "";
                if(value.type == 1) {
                    type = "Payment";
                ...
                trialBalanceInquiry += '<tr><td>'+type+'</td><td>'+value.counter+'</td><td>'+value.tran_date+'</td><td>'+value.account+'</td><td>'+value.amount+'</td><td>'+value.memo+'</td>="btn btn-success btn-sm" title="Edit"><i class="fa fa-eye" aria-hidden="true" title="Edit"></i></a></td></tr>';
            });                                
            $('#trial_balance').html(trialBalanceInquiry); 
        },error:function(response){
            alert('Error!!! Try again');
        } 			
    });
