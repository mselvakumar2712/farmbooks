...
<body>
      <div class="container-fluid pl-0 pr-0">
                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            ...
                            <li><a href="<?php echo base_url('staff/finance/debitnote');?>"class="active">Debit Note</a></li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="content mt-3">
			...
			<form action="<?php echo base_url('staff/finance/postDebitnote')?>" method="post" id="debit_note_form" name="sentMessage" novalidate="novalidate" >
                  <div class="row">
							<div class="col-md-12">
								<div class="card">
									<div class="card-body">
										<div class="container-fluid">
										  ...
										  <div class="row">
                                                <div class="form-group col-md-4">
													...
													<input class="form-control" type="date" id="entry_date" name="entry_date" required data-validation-required-message="provide the entry date" value="<?php echo date("Y-m-d"); ?>" max="<?php echo date("Y-m-d"); ?>">
											...
											<div class="row">
												...
												<div class="form-group col-md-2">
													<label for="inputAddress">Invoice Amount<span class="text-danger">*</span></label>
                                            ...
                                            <div class="form-group col-md-6">
													...
													<select  class="form-control" id="ledger_entry"   name="ledger_entry" style="width:100%" required="required" data-validation-required-message="Please select ledger entry">
														...
														<option  value="<?php echo $customer_list[$i]->debtor_no; ?>"><?php echo $customer_list[$i]->debtor_no.'-'.$customer_list[$i]->name; ?></option>
														...
														<option value="<?php echo $supplier_list[$i]->supplier_id; ?>"><?php echo $supplier_list[$i]->supplier_id.'-'.$supplier_list[$i]->supp_name; ?></option>
                                         ...
                                         <?php }else{?>
                                          <option  value="<?php echo $gl_trans_list[$i]->account_code.'-'.$gl_trans_list[$i]->account_name; ?>"><?php echo $gl_trans_list[$i]->account_code.'-'.$gl_trans_list[$i]->account_name; ?></option>			                                         
													  <?php } 
                                          }}?>
													</select>
													<p id="validate_ledger_entry" class="help-block text-danger"></p>
												</div>
												<div class="form-group col-md-4">
													<label for="inputAddress">Debit Note <span class="text-danger">*</span></label>
													<input type="text" class="form-control numberOnly" maxlength="6" id="debit_note" name="debit_note" placeholder="Debit Note " required="required" data-validation-required-message="Please enter debit note">
													<p id="validate_debit_note" class="help-block text-danger"></p>
												</div>
											</div>
											<div class="row ">
...
$("#sendMessageButton").click(function() {
	...
	var debit_note = document.getElementById("debit_note").value;
	...
	if(entry_date == ''){
		...
		$("#validate_debit_note").html("Please enter debit note.");
		return false;
...
$('#invoice_no').on('change', function () {
        var invoice= $(this).val();//var a=invoice.split(",");
		if(1 < invoice.length){
		var results = [];
...
function get_customername(gSelectedInv){
		...
		$.ajax({
			url: "<?php echo base_url();?>staff/finance/getCustomername",
			type: "POST",
			...
			success:function(response) { 
				...
				if(responseArray.type == 'supplier'){
					...
					document.getElementById('customer_name').value = value.supp_name;
					//document.getElementById('invoice_amt').value = value.ov_amount;
					...
					document.getElementById('customer_name').value = value.name;console.log(value.debtor_no);
					//document.getElementById('invoice_amt').value = value.ov_amount;
					document.getElementById('invoice_date').value = value.tran_date;
					var day=updateContraDay(value.tran_date);					
					document.getElementById("invoice_day").value = day;
...
function getinvoiceamount(invoice){
			...
			$.ajax({
			url: "<?php echo base_url();?>staff/finance/getInvoiceAmount",
			type: "POST",
...
$('.invoice_no').select2({
	...
	delay: 250,
	ajax: {
	  url: '<?php echo base_url('staff/finance/getInvoiceno')?>',
	  dataType: 'json',
	  delay: 250,
