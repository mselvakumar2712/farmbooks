			...
			<div class="breadcrumbs">
				...
				<div class="col-sm-8">
					<div class="page-header float-right">
                        ...
                        ?>
								<li><a class="active" href="<?php echo base_url('staff/Market/customers');?>">Customers</a></li>
                        <?php }else if($page_module == 'finance') {
                        ?>
								<li><a class="active" href="<?php echo base_url('staff/finance/customers');?>">Customers</a></li>
                        <?php }else {
                        ?>
								<li><a class="active" href="<?php echo base_url('staff/Market/customers');?>">Customers</a></li>
                        <?php }?>
            ...
            <?php }?>
				<div class="animated fadeIn">
                   ...
                   <?php 
                     if($page_module == 'market') {
                     ?>
                     <form id="addpopi_Form" method="POST" action="<?php echo base_url('staff/Market/postcustomer')?>" role="form" name="sentMessage" novalidate="novalidate" data-toggle="validator" method="post" accept-charset="utf-8">
                     <?php }else if($page_module == 'finance') {
                     ?>
                     <form id="addpopi_Form" method="POST" action="<?php echo base_url('staff/finance/postcustomer')?>" role="form" name="sentMessage" novalidate="novalidate" data-toggle="validator" method="post" accept-charset="utf-8">
                     <?php }else {
                     ?>
                     <form id="addpopi_Form" method="POST" action="<?php echo base_url('staff/Market/postcustomer')?>" role="form" name="sentMessage" novalidate="novalidate" data-toggle="validator" method="post" accept-charset="utf-8">
                     <?php }?>
									...
									<div id="form-step-0" class="form-horizontal" role="form" data-toggle="validator">
										...
										<div class="form-group col-md-4 ">
											...
											<input type="text" class="form-control" autocomplete="off" onkeyup="ajaxSearch()" maxlength="100"  id="customer_name" name="customer_name" placeholder="Customer Name" required="required" data-validation-required-message="Please enter customer name.">
											<div id="suggestions">
										...
										<div class="form-group col-md-4">
											...
											<input type="text" class="form-control numberOnly" maxlength="10" id="mobile_num" pattern="^[6-9]\d{9}$" name="mobile_num" placeholder="Mobile Number"  data-validation-required-message="Please enter mobile number.">
										...
										<div class="form-group col-md-4">
											...
											<?php //for($i=0; $i<count($sale_types);$i++) { ?>	
                                            <option value="<?php //echo $sale_types[$i]->id; ?>"><?php //echo $sale_types[$i]->sales_type; ?></option>
                                            <?php// } ?>
											</select>-->							
										</div>
										</div>
										...
										<div class="form-group col-md-4">
											<label for="inputAddress2">Mobile Number  </label>
											<input type="text" class="form-control numberOnly" maxlength="10" id="physical_mobile_num" pattern="^[6-9]\d{9}$" name="physical_mobile_num" placeholder="Mobile Number"  data-validation-required-message="Please enter mobile number.">
											<div id="mbl_validate" class="help-block with-errors text-danger"></div>
										</div>
										<div class="form-group col-md-4">
											<label for="inputAddress2">E-Mail Address </label>
                                       ...
                                       <select id="bank_info" name="bank_info" class="form-control">
										...
										<div class="col-md-6">
											<div class="form-group col-md-6" >
												<label for="inputAddress2">Opening Balance <span class="text-danger">*</span></label>
												<input type="text" class="form-control numberOnly" maxlength="6" name="opening_balance" id="opening_balance" required placeholder="Opening Balance" data-validation-required-message="Provide the opening balance">
												<div class="help-block with-errors text-danger"></div>
											</div>
											<div class="form-group col-md-6">
...
function ajaxSearch() {
    ...
    if (input_data.length < 3) {
      ...
      $('#addpopi_Form').find('input[type=text], select').each(function(){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>staff/market/getAllCustomersSuppliers",
            data: post_data,
 ...
 function fnCustomer(element, id) {
	...
	$.ajax({
		type: "POST",
		url: "<?php echo base_url(); ?>staff/market/getCustomer",
		data: post_data,
 ...
 function fnSupplier(element, id) {
	...
	$.ajax({
		type: "POST",
		url: "<?php echo base_url(); ?>staff/market/getSupplier",
		data: post_data,
		success: function (response) {
			responsearr = $.parseJSON(response);
			$.each(responsearr, function(key, value) {
...
function CopyAdd() {
	...
	function activeClick(customerval) {
				if(customerval.checked == true) { 
					...
					$.ajax({
						url: '<?php echo base_url('staff/market/getallcustomer')?>',
						type: "GET",
						success:function(response) {
							responsearr=$.parseJSON(response);
					...
					$("#customer option").remove() ;		
					$.ajax({
						url: '<?php echo base_url('staff/market/getactivecustomer')?>',
						type: "GET",
						success:function(response) {
							responsearr=$.parseJSON(response);
							var div_data = '<option value="0">New Customer</option>';
...
$('select[name="customer"]').on('change', function(e) {
       ...
       <?php 
         if($page_module == 'market') {
         ?>
         window.location = "<?php echo base_url();?>staff/Market/customeredit/"+customer;    
         <?php }else if($page_module == 'finance') {
         ?>
         window.location = "<?php echo base_url();?>staff/finance/customeredit/"+customer;          
         <?php }else {
         ?>
         window.location = "<?php echo base_url();?>staff/Market/customeredit/"+customer;  
         <?php }?>
...
$('#pin_code').focusout(function(e) 
{
   ...
   function getbanklist(state_id) {
      $("#bank_name option").remove();
			$.ajax({
				url:"<?php echo base_url();?>staff/market/getbanklist/"+state_id,
				type:"GET",
				data:"",
				dataType:"html",
				cache:false,			
   ...
   function getBankAddressByBankName( bank_name_id ) {
    ...
    if(bank_name_id != ''){    
	   $.ajax({
		  url:"<?php echo base_url();?>staff/market/getBankAddressByBankName",
		  type:"POST",
 ...
 function getplaceofcustomer(state_id) {
       ...
       var state_id = state_id;
			...
			$.ajax({
				url:"<?php echo base_url();?>staff/inventory/getplacesupply/",
				type:"GET",
				data:"",
				dataType:"html",
				cache:false,			
