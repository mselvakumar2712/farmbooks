            ...
            <?php }?>
				<div class="animated fadeIn">
					<div class="card">
                     ...
                     if($page_module == 'market') {
                     ?>
                     <form id="addpopi_Form" method="POST" action="<?php echo base_url('staff/Market/postupdatecustomer/'.$customer_info[0]->debtor_no)?>" role="form" name="sentMessage" novalidate="novalidate" data-toggle="validator" accept-charset="utf-8">
                     <?php }else if($page_module == 'finance') {
                     ?>
                     <form id="addpopi_Form" method="POST" action="<?php echo base_url('staff/finance/postupdatecustomer/'.$customer_info[0]->debtor_no)?>" role="form" name="sentMessage" novalidate="novalidate" data-toggle="validator" accept-charset="utf-8">
                     <?php }else {
                     ?>
                     <form id="addpopi_Form" method="POST" action="<?php echo base_url('staff/Market/postupdatecustomer/'.$customer_info[0]->debtor_no)?>" role="form" name="sentMessage" novalidate="novalidate" data-toggle="validator" accept-charset="utf-8">
                     <?php }?>						
                          ...
                          <input type="hidden" class="form-control" id="debtor_no" name="debtor_no" value="<?php echo $customer_info[0]->debtor_no; ?>">                                    
                              ...
                              <div class="form-group col-md-3">
                                 ...
                                 <select id="bank_info" name="bank_info" class="form-control" >
										...
										<div class="col-md-6">
											<div class="form-group col-md-6">
												...
												<input type="text" class="form-control numberOnly" maxlength="6" name="opening_balance" value="<?php echo $customer_info[0]->opening_balance; ?>" id="opening_balance" placeholder="Opening Balance" required data-validation-required-message="Provide the opening balance">
												<div class="help-block with-errors text-danger"></div>
											</div>
											<div class="form-group col-md-6">
                                ...
                                <?php 
                                 if($page_module == 'market') {
                                 ?>
                              <a href="<?php echo base_url('staff/market/customers');?>" id="cancel" class="btn btn-outline-dark btn-fs ml-2"> <i class="fa fa-close" aria-hidden="true"></i> Cancel</a>
                                 <?php }else if($page_module == 'finance') {
                                 ?>
                              <a href="<?php echo base_url('staff/finance/customers');?>" id="cancel" class="btn btn-outline-dark btn-fs ml-2"> <i class="fa fa-close" aria-hidden="true"></i> Cancel</a>
                                 <?php }else {
                                 ?>
                              <a href="<?php echo base_url('staff/market/customers');?>" id="cancel" class="btn btn-outline-dark btn-fs ml-2"> <i class="fa fa-close" aria-hidden="true"></i> Cancel</a>
                                 <?php }?>
...
function getBankAddressByBankName( bank_name_id ) {
    ...
    if(bank_name_id != ''){    
	   $.ajax({
		  url:"<?php echo base_url();?>staff/market/getBankAddressByBankName/"+bank_name_id,
		  type:"GET",
           data:"",
		  dataType:"html",
		  cache:false,			
...
function validateEmail(emailField){
	...
	function activeClick(customerval) {
		...
		/* $('select[name="customer"]').on('change', function(e) {
			...
			if(customer) { 
			$.ajax({
				url: "<?php echo base_url();?>staff/Market/editcustomer/"+customer,
				type: "POST",
