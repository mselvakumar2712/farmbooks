...
<?php 
	...
	if(isset($debtor_trans_last_record->trans_no)) {
		...
		$voucherNo = $fpoName."SL".$lastTransNo;
	}
	else {
		$lastTransNo = 1;
		$lastTransNo = str_pad($lastTransNo, 4, "0", STR_PAD_LEFT);
                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                             ...
                             <li><a href="<?php if($page_module == 'market') { echo base_url('staff/market/salesentry'); } else if($page_module == 'finance') { echo base_url('staff/finance/salesentry'); } ?>"class="active">Sales</a></li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="content mt-3">
		   <?php if($this->session->flashdata('success')){ ?>
            ...
            <div class="animated fadeIn">
			    <form method="POST" action="<?php if($page_module == 'market') { echo base_url('staff/market/postsales_entry'); } else if($page_module == 'finance') { echo base_url('staff/finance/postsales_entry'); } ?>" id="figform" name="sentMessage" novalidate="novalidate" >
                  <div class="row">
							<div class="col-md-12">
								<div class="card">
									<div class="card-body">
										<div class="container-fluid">
 										    <div class="row">
												<div class="form-group col-md-4">
													...
													<?php if($invoice_prefix =='no'){?>
														<a href="<?php echo base_url('staff/setting/invoiceprefix');?>" class="text-success">Click here to add invoice prefix</a>
													<?php }?>
												...
												<div class="form-group col-md-4">
													...
													<input class="form-control" type="date" id="invoiceDate" name="invoiceDate" required="required" data-validation-required-message="Please select invoice date." value="<?php echo date("Y-m-d");?>">
											...
											<div class="row">
												<div class="form-group col-md-3">
													...
													<input type="text" class="form-control" id="voucherNo" name="voucherNo" value="<?php echo $voucherNo; ?>" readonly placeholder="Voucher No." required="required" data-validation-required-message="Please enter voucher no.">
													<p class="help-block text-danger"></p>
												</div>
												<div class="form-group col-md-3">
											...
											<div class="table-responsive mt-3">  
                                             ...
                                             <th class="text-center" rowspan="2"></th>
                                                   ...
                                                   <option value="new_item">Add New Item</option>
															...
															<td width="10%" class="text-center">
																<input type="text" name="price[]" id="price0" class="form-control text-right numberOnly" maxlength="6" />
															</td>
                                             ...
                                             <td width="5%" class="text-center">
													...
													<tr>
														...
														<td colspan="2" ><input type="text" id="adjustment" name="adjustment"  maxlength="9" class="form-control text-right" placeholder="Adjustment" pattern="^\-?\d*(\.\d{0,2})?$" /></td>
													</tr>
													<tr class="total">
														<td colspan="7" class="text-right"> Total ( <span class="fa fa-inr"></span> )</td>
														<td colspan="2"><input type="text" id="grandTotal" name="grandTotal" readonly class="form-control text-right" /></td>
													</tr>
													</tbody>
												</table>  
											</div>
...
<script type="text/javascript">
	...
	/* function verifyInvoiceNumber(invoicenumber) {
		if(invoicenumber) {
			$.ajax({
			  url:"<?php echo base_url();?>staff/market/checkinvoiceno/"+invoicenumber,
			  type:"GET",
			  data:"",
			  dataType:"html",
			  cache:false,      
	...
	$(document).ready(function() {
		...
		$('#dynamic_field').on('change', 'tr select[name="item_description[]"]', function () {
         ...
         if($(this).val() != '') {
            ...
            if(customerId == '4020501' || customerId == '4020502')
               var deliveryToState = supplyLocationState;	
            else
		...
		$(document).on('click', '.btn_add', function(){ 
			var validate = true;
			$(this).parents('tr').find('input[type=text], select').each(function(){
				if($(this).attr('name') == "each_qty[]" || $(this).attr('name') == "each_unit[]" || $(this).attr('name') == "line_discount[]"){
				}
				else {
					if($(this).val() == ""){
						validate = false;
			...
			if(validate){
				...
				str_add += '	</td>';
				str_add += '	<td width="10%" class="text-center">';
				str_add += '		<input type="text" id="price'+i+'" name="price[]" class="form-control text-right numberOnly" maxlength="6" />';
				str_add += '	</td>';
            ...
            str_add += '	<td width="5%" class="text-center">';
				...
				$.ajax({
					url: '<?php echo base_url('staff/market/getproducts')?>',
					type: "GET",
					success:function(response) {
		...
		$('#customerId').change(function(){
			...
			if(debtor_no == 'new_customer') {
				window.location = "<?php echo base_url('staff/market/customers')?>";
			}
		...
		function getcustomerDetail( debtor_no ) {
		   $("#deliveryTo option").remove();
				...
				{   
					$.ajax({
						url:"<?php echo base_url();?>staff/market/branchdetail/"+debtor_no,
						type:"GET",
						data:"",
						dataType:"html",
						cache:false,			
						success:function(response) {
							response=response.trim();                
							responseArray=$.parseJSON(response);console.log(responseArray);
							if(responseArray.status == 1){
