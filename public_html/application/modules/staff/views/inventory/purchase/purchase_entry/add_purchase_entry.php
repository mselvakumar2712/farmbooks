...
#remove_0 {
  ...
  <body <?php  if($page_title == 'Purchase Entry' and $page_module == 'inventory'){echo 'class="open"';}else if($page_title == 'Purchase Entry'  and $page_module == 'finance'){echo 'class="open"';}?>>
      <div class="container-fluid pl-0 pr-0">
        ...
        <div class="content mt-3">
			...
			<?php 
				...
				?>
           <form action="<?php echo base_url('staff/inventory/post_purchase_entry')?>" method="post" id="figform" name="sentMessage" novalidate="novalidate" >           
			<?php } else if($page_module == 'finance') {
				?>
				  <form action="<?php echo base_url('staff/finance/post_purchase_entry')?>" method="post" id="figform" name="sentMessage" novalidate="novalidate" >           
			<?php } else {
				?>
				  <form action="<?php echo base_url('staff/inventory/post_purchase_entry')?>" method="post" id="figform" name="sentMessage" novalidate="novalidate" >           
			<?php }?>
								...
								<div class="card">
									<div class="card-body">
	                                 ...
	                                 <div class="form-group col-md-3">
													...
													<select class="form-control" id="supplier" name="supplier" required="required"  style="width:100%" data-validation-required-message="Please select supplier.">
                                          ...
                                          <option value="new_supplier">Add New Supplier </option>
  												...
  												<div class="form-group col-md-3">
												    ...
												    <input type="text" class="form-control" maxlength="15" id="voucher_no" name="voucher_no" value="<?php echo ucwords($last_voucher_no);?>"   readonly placeholder="Voucher No" required="required" data-validation-required-message="Please enter voucher no.">
													<p class="help-block text-danger"></p>
											    </div>	
                                    <div class="form-group col-md-3">
												    <label for="inputAddress2">Supplier Invoice No.<span class="text-danger">*</span></label>
												    <input type="text" class="form-control" maxlength="15" id="invoice_no" name="invoice_no" placeholder="Supplier Invoice No" onfocusout="checkSupplierInvoiceNo()" required="required" data-validation-required-message="Please enter supplier invoice no.">
												...
												<table class="table table-bordered low-padded" id="dynamic_field">
													<thead>
														<tr>
															...
															<th class="text-center" rowspan="2">Unit Price<br>( <span class="fa fa-inr"></span> )</th>
													...
													<tr id="row0">  
														...
														<td width="20%"><select class="form-control" id="item_description0"  name="item_description[]" required="required" data-validation-required-message="Please select item description.">
														...
														<td width="8%"><input type="text" id="qty0" name="qty[]"  maxlength="6" class="form-control name_list numberOnly text-right" required="required" /><p class="help-block text-danger"></p></td>  
														<td width="8%"><input type="text" id="invoiced0"  name="invoiced[]"  maxlength="6" class="form-control name_list textOnly text-right" required="required" /><p class="help-block text-danger"></p></td>  
														...
														<p class="help-block text-danger"></p></td>
														<td width="8%"><input type="text" id="package_qty0" name="package_qty[]"  maxlength="6" class="form-control name_list numberOnly text-right" required="required" /><p class="help-block text-danger"></p></td>  														
														...
														<td width="10%"><input type="text" id="price0" name="price[]" maxlength="6" class="form-control name_list numberOnly text-right" required="" /><p class="help-block text-danger"></p></td>  
														<td width="9%"><input type="text" id="line_discount0" name="line_discount[]" maxlength="6"  class="form-control name_list numberOnly text-right" required="" /><p class="help-block text-danger"></p></td>  																											
													...
													<tr>
														...
														<td colspan="2" ><div id="divCGSTTotal"><input type="text" id="cgst"  name="cgst"  readonly class="form-control text-right"  placeholder="CGST" /></div></td>							
														<td colspan="4" class="text-right">Delivery Charge( <span class="fa fa-inr"></span> )</td>
														<td colspan="2" ><input type="text" id="delivery_charge"  name="delivery_charge"  maxlength="6" class="form-control numberOnly text-right" placeholder="Delivery Charge" /><div class="lblIncludeGST" ><label for="includeGst">GST Included</label> </div>&nbsp;<div class="includeGST"><input type="checkbox" id="includeGst" checked name="includeGst" value="1" class="text-center mt-2"></div></td>
														<?php
															} 
															else {
															?>
													...
													<tr>
														...
														<td colspan="2" ><div id="divSGSTTotal"><div id="divSGSTTotal"><input type="text" id="sgst"  name="sgst"  readonly class="form-control text-right"  placeholder="SGST/UTGST" /></div></td>
										 ...
										 <div class="form-row">
											  ...
											  <div id="success"></div>
												...
												if($page_module == 'inventory') {
												?>
												<a href="<?php echo base_url('staff/inventory/purchaseentry');?>" class="btn btn-outline-dark">Cancel</a>	
											 <?php 
												}else if($page_module == 'finance') {
												?>
													<a href="<?php echo base_url('staff/finance/purchaseentry');?>" class="btn btn-outline-dark">Cancel</a>	
												<?php }else {
												?>
													<a href="<?php echo base_url('staff/inventory/purchaseentry');?>" class="btn btn-outline-dark">Cancel</a>	
												<?php }?>
...
$(document).ready(function(){
    $('#supplier').on('change', function(e){
     ...
     $('#select2-item_description0-container').html('Description');
	...
	function calculateGrandTotal() {
		...
		if(gstEnable == 1 && $('#includeGst').is(':checked')) {
			var gstShipping = parseFloat(shippingCharge || 0) * ( 5/100 );
	...
	function getLineTaxCalculation(gstType, itemId, ind){
		$.ajax({
			url: "<?php echo base_url();?>staff/inventory/getHsncode",
			type: "POST",
			data: {item_id: itemId},
			dataType: "html",
			cache: false,      
	...
	$('#dynamic_field').on('change', 'tr select[name="item_description[]"]', function () {
		if($(this).val() == "new_item") {
         window.location = "<?php echo base_url('staff/masterdata/addproductfpo')?>";
      }
	...
	/*$('#dynamic_field').on('change', '#adjustment', function () {
			...
			if(adjust_val > amount_val){
				swal('',"Adjustment sholud not be greater than total");
			}
	});
	...
	$('#dynamic_field').on('keyup', 'tr input[name="line_discount[]"]', function () {
			...
			if(discount > line_total){
				...
				swal('',"Discount sholud not be greater than line total");
			}
	});
	...
	$('#dynamic_field').on('change', 'tr input[name="invoiced[]"]', function () {
			...
			if(invoiced > line_qty){
				...
				swal('',"Quantity Invoiced sholud not be greater than quantity received");
			}
	});
	...
	$(document).ready(function() {
		...
		$('#supplier').on('change', function () {
         if($(this).val() == 'new_supplier') {
            window.location = "<?php echo base_url('staff/inventory/suppliers')?>";
         }   
           ...
           window.location = "<?php echo base_url('staff/masterdata/addproductfpo')?>";
            ...
            if(supply == "" || supply == "new_supplier"){
               ...
               if($(obj).attr('name') == 'item_description[]') {
                  var str = $(obj).attr('id');
                  var ind = str.replace('item_description','');
                  $('#select2-item_description'+ind+'-container').html('Description');
               }
		...
		$('#dynamic_field').find('tr input[id=item_description'+i+'],tr input[id=qty'+i+'],tr input[id=invoiced'+i+'],tr input[id=price'+i+'],tr select[id=unit'+i+'],tr input[id=line_total'+i+']').each(function(){
      ...
      /*$('#dynamic_field').on('change','select', function () {
		...
		str_add += '<option value="">Description</option>';
		str_add += '</select>';	
		str_add += '</td>';
		...
		$.ajax({
			url: '<?php echo base_url('staff/inventory/getproducts')?>',
			type: "GET",
			success:function(response) {
		...
		$.ajax({
			url: '<?php echo base_url('staff/inventory/getquantityunit')?>',
			type: "GET",
			success:function(response) {
				responsearr=$.parseJSON(response);
      ...
      $(document).on('click', '#sendMessageButton', function() {
			...
			if($('#supplier').val() == '' || $('#invoice_no').val() == '' || $('#delivery_to').val() == '') {
				...
				if($('#item_description'+lastLineIndex).val() == '' || $('#qty'+lastLineIndex).val() == '' || $('#invoiced'+lastLineIndex).val() == '' || $('#unit'+lastLineIndex).val() == '' || $('#price'+lastLineIndex).val() == '') {
					formValidate = false;
				}
			}
			else if(lastLineIndex >0) {
   ...
   function verifyInvoiceNumber(invoicenumber) {
      if(invoicenumber) {
         $.ajax({
           url:"<?php echo base_url();?>staff/inventory/checkinvoiceno/"+invoicenumber,
           type:"GET",
           data:"",
           dataType:"html",
           cache:false,      
   ...
   function checkSupplierInvoiceNo() {
      ...
      if(invoiceNo != '' && supplierId != '') {
         ...
         $.ajax({
           url: "<?php echo base_url();?>staff/inventory/checkSupplierInvoiceNo",
           method: "POST",
           data: data,
           cache: false,
           dataType: "html",          
