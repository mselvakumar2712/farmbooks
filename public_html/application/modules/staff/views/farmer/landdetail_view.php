...
<!--									<a href="#" id="" class="del btn btn-danger">Delete</a>-->
									<a href="<?php echo base_url('staff/farmer/landlist');?>" id="ok" class="btn btn-fs btn-outline-dark"><i class="fa fa-arrow-circle-left" aria-hidden="true"></i> Back</a>	
									<a href="<?php echo base_url('staff/farmer/landlist');?>" id="cancel" class="btn btn-fs btn-outline-dark" style="display:none;"><i class="fa fa-close" aria-hidden="true"></i> Cancel</a>							
								</div>
						       </form>
						     </div>
					      </div>
...
//                  $("#both_organic_certifiation").prop('required',true);
			...
			$('a.del').click(function() {
                var land_id = <?php echo $_REQUEST['id']?>;
                swal({
                 title: 'Are you sure?',
                 ...
                 if (result.value) {          
                    ...
                    $.ajax({
                      url: "<?php echo base_url();?>staff/farmer/deleteLandDetail/"+land_id,
                      type: "POST",
                      data: {
                         this_delete: land_id,
                      },
                      ...
                      success: function() {        
                         setTimeout(function() {
                          window.location.replace("<?php echo base_url('staff/farmer/landlist')?>");
                         }, 1000);
                      },
                      error: function() {
    ...
    //jQuery method
      $.ajax({
        url:"<?php echo base_url();?>staff/farmer/editland/"+land_id,
        type:"GET",
        dataType:"html",
        cache:false,      
        success:function(response){      
            ...
            if(responseArray.status == 1){                                                
                ...
                if(responseArray.land_list[0].land_holdings==1){
                    ...
                    var href_value="<?php echo base_url('staff/farmer/addland?id=');?>"+responseArray.land_list[0].farmer_id;
                                ...
                                if(radios1.is(':checked') === false) {
                                    ...
                                    if(responseArray.land_list[0].organic_former==1 || responseArray.land_list[0].organic_former==3 ){
                                        ...
                                        document.getElementById("effective_period_certifiation_from").value=responseArray.land_list[0].certifiation_period_from; 
                                         document.getElementById("effective_period_certifiation_to").value=responseArray.land_list[0].certifiation_period_to; 
                                         var area_uom = responseArray.land_list[0].area_uom;
                                         setTimeout(function(){                                                                 //document.getElementById("cultivation_area_uom").value = area_uom;  
                                         }, 500);                                                 
                                        }
                                    ...
                                    document.getElementById("both_survey_number").value=responseArray.land_list[0].survey_number;
                                        ...
                                        if(responseArray.land_list[0].address == 1){                
                                            if(farm_address.is(':checked') === false) {
                                                farm_address.filter('[value=1]').prop('checked', true);
												$('#both_farm_address').show();
                                                getBothLandPincode(responseArray.land_list[0].pin_code); document.getElementById("both_pincode").value=responseArray.land_list[0].pin_code;
...
$('form').submit(function(e){
        ...
        $.ajax({
            url:"<?php echo base_url();?>staff/farmer/updateland/"+land_id,
            type:"POST",
            data:farmerdata,
            dataType:"html",
            cache:false,      
            success:function(response){      
              ...
              console.log(response);
                if(responseArray.status == 1) {
                     ...
                     setTimeout(function(){ 
                       window.location = "<?php echo base_url('staff/farmer/landlist')?>";
                     }, 500);					
				} else {
...
function getFarmerProfileLocation(type){
	...
	if(type == 1){
		...
		var url = "<?php echo base_url();?>staff/farmer/edit_Farmer/"+farmer_id;
	} else {
		var land_id = '<?php echo $_REQUEST['id']?>';
		var url = "<?php echo base_url();?>staff/farmer/editland/"+land_id;
	}
