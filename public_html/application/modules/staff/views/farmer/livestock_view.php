...
<!--									<a href="#" id="" class="del btn btn-danger">Delete</a>-->
									<a href="<?php echo base_url('staff/farmer/livestocklist');?>" id="ok" class="btn btn-fs btn-outline-dark"><i class="fa fa-arrow-circle-left"></i> Back</a>	
									<a href="<?php echo base_url('staff/farmer/livestocklist');?>" id="cancel" class="btn btn-fs btn-outline-dark" style="display:none;"><i class="fa fa-close" aria-hidden="true"></i> Cancel</a>
								</div>
						       </form>
						     </div>
					      </div>
			...
			$('a.del').click(function() {
                ...
                swal({
                 ...
                 if (result.value) {          
                    ...
                    $.ajax({
                      url: "<?php echo base_url();?>staff/farmer/deleteLiveStock/"+livestock_id,
                      type: "POST",
                      data: {
                      ...
                      success: function() {        
                         setTimeout(function() {
                          window.location.replace("<?php echo base_url('staff/farmer/livestocklist')?>");
                         }, 1000);
                      },
                      error: function() {
   ...
   $(document).ready(function(){
       $('select[name="live_stocks"]').on('change', function() {
        ...
        $('input').on('click', function() {
            ...
            // jQuery method
			$.ajax({
			url:  "<?php echo base_url();?>staff/farmer/editlivestock/"+livestock_id,
			type: "GET",
			dataType:"html",
			cache:false,			
			success:function(response){		  
                ...
                if(responseArray.status == 1){
                    ...
                    var href_value="<?php echo base_url('staff/farmer/addlivestock?id=');?>"+responseArray.livestock_list[0].farmer_id;
...
$('form').submit(function(e){
        ...
        $.ajax({
            url:"<?php echo base_url();?>staff/farmer/updatelivestock/"+livestock_id,
            type:"POST",
            data:farmerdata,
            dataType:"html",
            cache:false,      
