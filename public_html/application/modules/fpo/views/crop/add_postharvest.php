                    ...
                    <div class="card">
                        <div class="card-body">
									<div class="container-fluid">
										<form name="sentMessage" method="post" action="<?php echo base_url('fpo/updatecrop/post_harvest'); ?>" novalidate="novalidate" id="add_postharvest">
											<div class="row row-space">
												<div class="form-group col-md-6">
													<label for="inputAddress2">Farmer Name <span class="text-danger">*</span></label>
													<select id="farmer_id" name="farmer_id" class="form-control" required="required" data-validation-required-message="Please select farmer" >
												...
												<div class="form-group col-md-6 ">
													...
													<input type="date" class="form-control" name="date" id="date" placeholder="Date" required="required" data-validation-required-message="Please enter date of formation." title="If need date picker, click the arrow" value="<?php echo date('Y-m-d'); ?>" min="2018-01-01" max="2050-12-31">
													<p class="help-block text-danger"></p>
												</div>
											</div>
											...
											<div class="row rowspace">
													...
													<div class="form-group col-md-2 ">
														...
														<input type="text" class="form-control numberOnly" maxlength="6" name="input_qty" required="required" id="input_qty" placeholder="000" required="required" data-validation-required-message="Please enter input(qty)" >
														<p class="help-block with-errors text-danger"></p>
													</div>
												 ...
												 <div class="form-group col-md-3">
													...
													<input type="text" class="form-control numberOnly text-right" maxlength="6" name="cost_post_harvesting" id="cost_post_harvesting" placeholder="0.00">
 												</div>
											</div>
											<div class="row rowspace">
												<div class="col-md-8" id="div1"></div>
												<div class="col-md-4" id="div2" style="display:none;"></div>
											</div>
			...
			success:function(response) {
				...
				if(responseArray.status==1){
					if(Object.keys(responseArray).length > 0) {
						$("#variety").append($('<option></option>').val('').html('Select Variety'));
					} else {
						$("#variety").append($('<option></option>').val('').html(''));  
					}
	...
	if(crop_id!=''){
		...
		$.ajax({
			...
			success:function(response){
				...
				if(responseArray.status==1){
					if(Object.keys(responseArray).length > 0) {
						$("#input").append($('<option></option>').val('').html('Select Input'));
					} else {
					...
					$.each(responseArray.input_list,function(key,value){
						...
						for(i=0;i<crop_output.length;i++){
							$("#input").append($('<option></option>').val(value.name_id).html(crop_output[i]));						
						}
					});
				} 
			},
...
function getoutputDetail(input){
	if(input!=''){
			...
			success:function(response){
				...
				if(responseArray.output_list.length != 0){	
					...
					$.each(responseArray.output_list,function(key,value){
						...
						sale_list += '<tbody id="output_field"><tr id="row_0" class="row_child row-space mb-5"><td class="col-md-4 form-group"><input type="text" id="quantity_uom" onfocusout="myFunction(this.value)" value='+value.qty_sales+' name="quantity_uom[]" class="form-control "  ></td><td class="col-md-8 form-group"> <label for="inputAddress">Quantity UOM</label><select id="sales_qty_uom"  name="sales_qty_uom[]"  class="form-control"><option value="" >Select</option>';
						<?php for($i=0;$i<count($quantity_uom);$i++) { ?>
					...
					output_list += '<tbody id="dynamic_field"><tr id="row0" class="row row-space"><td class="col-md-4 form-group"><label for="inputAddress">Output <span class="text-danger">*</span></label><select type="text" id="output_product" name="output_product[]" class="form-control make"></select></td><td class="col-md-3 form-group"><label for="inputAddress2">Output(Qty) <span class="text-danger">*</span></label><input type="text" id="output_qty" name="output_qty[]" class="form-control" required placeholder="000" maxlength="6"></td><td class="col-md-3 form-group"> <label for="inputAddress">Quantity UOM <span class="text-danger">*</span></label><select id="output_quantity_uom" name="output_quantity_uom[]" class="form-control" required ><option value="">Select UOM</option>';
					<?php for($i=0;$i<count($quantity_uom);$i++) { ?>
					...
					sale_list += '<tbody id="output_field"><tr id="row_0" class="row row-space mb-5"><td class="col-md-6"><label for="inputAddress">Sales (Qty)</label> <input type="text" id="quantity_uom" onfocusout="myFunction(this.value)" name="quantity_uom[]" class="form-control" placeholder="000" maxlength="6"></td><td class="col-md-6"> <label for="inputAddress">Quantity UOM</label><select id="sales_qty_uom" name="sales_qty_uom[]" class="form-control"><option value="">Select UOM</option>';
					<?php for($i=0;$i<count($quantity_uom);$i++) { ?>
        ...
        swal('Sorry!', 'Please provide input field');
    }
}
