            ...
            <div class="animated fadeIn">
                ...
                <div class="col-md-12">
                    <div class="card">
                     ...
                     <form id="crop_standardForm" action="<?php echo base_url('fpo/crop/updatecrop/').$croplist['id']; ?>" name="sentMessage" method="post" novalidate="novalidate" enctype="multipart/form-data">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select id="farmer_id" name="farmer_id" class="form-control" required="required" data-validation-required-message="Please select farmer" data-search="true" readonly>
										...
										<?php for($i=0; $i<count($farmers);$i++) { 
											if($farmers[$i]->id == $croplist['farmer_id']){
										?>										
                                ...
                                <div class="form-group col-md-6">
                                      ...
                                      <div class="row-fluid">
                                        <div class="col-md-4">
                                          <div class="form-check-inline form-check">
                                            ...
                                            <input type="radio" id="crop_type" name="crop_type" value="1" class="form-check-input" <?php if($croplist['crop_type'] == 1){echo "checked";} ?> required><span class="ml-1">Regular</span>
                                            </label>
                                          </div> 
                                        </div>
                                        <div class="col-md-4">
                                          <div class="form-check-inline form-check">
                                            ...
                                            <input type="radio" id="crop_type" name="crop_type" value="2" class="form-check-input" <?php if($croplist['crop_type'] == 2){echo "checked";} ?> required><span class="ml-1">Multi Crop</span>
                                            </label>
                                           </div>
                                        </div>	
                                        <div class="col-md-4">
                                          <div class="form-check-inline form-check">
                                            <label for="inline-radio2" class="form-check-label">                         
                                            <input type="radio" id="crop_type" name="crop_type" value="3" class="form-check-input" <?php if($croplist['crop_type'] == 3){echo "checked";} ?> required><span class="ml-1">Intercrop</span>
                                            </label>
                                           </div>
                                        </div>										
                                      </div>
                                 ...
                                 <div class="form-group col-md-6">
                                     ...
                                     <?php for($i=0; $i<count($category);$i++){ 
										if($category[$i]->id == $croplist['category_id']){
									 ?>										
                           ...
                           <div class="row row-space">
                            ...
                            <div class="row row-space">							                                          
                                ...
                                <div class="form-group col-md-3">                               
									...
									<input type="date" class="form-control" name="seed_date" id="seed_date" placeholder="Date of seed plant" data-validation-required-message="Please enter date of seed plant" title="If need date picker, click the arrow" value="<?php echo $croplist['seedling_date']; ?>" min="<?php echo $beforeMonth; ?>" max="<?php echo $afterMonth; ?>" onfocusout="calculateExpectedHarverst(this.value);" >
                                    <p class="help-block text-danger"></p>
                                </div>
								<div class="form-group col-md-2">
                                    <div class="form-check-inline form-check mt-3">
                                    <label for="inline-radio1" class="form-check-label"></label>
                                    <input type="checkbox" id="direct_seed" name="direct_seed" value="1" class="form-check-input" <?php if($croplist['is_direct_seeding'] == 1){echo "checked";} ?>>Direct Seedling
                                    </div>                                    
                                </div>
								<div class="form-group col-md-4">
                                    <label for="inputAddress2">Date of Transplanting </label>
                                    <input type="date" class="form-control" name="transplant_date" id="transplant_date" placeholder="Date of Transplanting" title="If need date picker, click the arrow" value="<?php echo ($croplist['is_direct_seeding'] == 0)?$croplist['transplant_date']:""; ?>" min="<?php echo $beforeMonth; ?>" max="<?php echo $afterMonth; ?>" onfocusout="calculateExpectedHarverst(this.value);" >
									<p class="help-block text-danger"></p>
                                </div>
                           </div>
                            <div class="row row-space">
								<div class="form-group col-md-3">
									...
									<input type="text" class="form-control numberonly" id="area" maxlength="3" minlength="" name="area" placeholder="000" required value="<?php echo $croplist['area']; ?>">
									<p class="help-block text-danger"></p>
                                </div>
								<div class="form-group col-md-3">
								...
								<div class="form-group col-md-3">
									...
									<input type="text" class="form-control numberonly" maxlength="4" id="used_seed_qty" name="used_seed_qty" placeholder="000" data-validation-required-message="Please enter seed quantity" required value="<?php echo $croplist['seed_qty']; ?>">
								    <p class="help-block text-danger"></p>
                                </div>
								<div class="form-group col-md-3">
                            ...
                            <div class="row row-space">                                
								...
								<div class="form-group col-md-2">
								  ...
								  <input type="text" class="form-control numberonly text-right" id="seed_cost" step="0.01" name="seed_cost" placeholder="0.00" value="<?php echo number_format($croplist['seed_cost'], 2, '.', ''); ?>">
								  <p class="help-block text-danger"></p>
								</div>
								<div class="form-group col-md-4">
                                    <label for="inputAddress2">Expected Date of Harvest <span class="text-danger">*</span></label>
                                    <input type="date" class="form-control" name="exp_harvest_date" id="exp_harvest_date" placeholder="Expected Date of Harvest" required="required" data-validation-required-message="Please enter expected date of harvest" title="If need date picker, click the arrow" value="<?php echo $croplist['expected_harvest_date']; ?>" readonly>
									<p class="help-block text-danger"></p>
                                </div>
                           </div>
                           <div class="row row-space">                           
                        ...
                        <div class="col-md-12 text-center">
                           <div id="success"></div>
							...
							<a href="<?php echo base_url('fpo/crop');?>" id="cancel" class="btn btn-fs btn-outline-dark"><i class="fa fa-close" aria-hidden="true"></i> Cancel</a>
						</div>                        
					</div>
					</form>
					</div>
...
$(document).ready(function(){
	...
	var crop_category = "<?php echo $croplist['category_id']; ?>";
	var crop_subcategory = "<?php echo $croplist['subcategory_id']; ?>";
	var crop_name = "<?php echo $croplist['crop_id']; ?>";
	var variety_id = "<?php echo $croplist['variety_id']; ?>";
	getCropDetail(crop_category, crop_subcategory);	
	getsubcategoryDetail(crop_subcategory, crop_name);	
	getCropnameDetail(crop_name, variety_id);
	
	var is_direct_seeding = "<?php echo $croplist['is_direct_seeding']; ?>";
	if(is_direct_seeding == 1){
	       ...
	       //edit crop API CALL
			...
			success:function(response){		  
                ...
                if(responseArray.status == 1 && responseArray.crop_list.length > 0){
                  ...
                  //console.log('yes',responseArray.crop_list[0].subcategory_id) 
...
function getCropDetail(category_id, sub_category_id){
    if(category_id != ''){   
        $.ajax({
                ...
                if(responseArray.status == 1){
					...
					$.each(responseArray.subcategory_list,function(key,value){					
						if(value.id == sub_category_id){
							subcategory_list += '<option value='+value.id+' selected>'+value.name+'</option>';
						} else {
		...
		swal('Sorry!', 'Please select category field');
	}
}
		
	
function getsubcategoryDetail(subcategory_id, crop_id){      
    if(subcategory_id!=''){   
        $.ajax({
			...
			success:function(response) {
                ...
                if(responseArray.status == 1){
					...
					$.each(responseArray.name_list,function(key,value){					
						if(crop_id == value.id){
							name_list += '<option value='+value.id+' selected>'+value.name+'</option>';
						} else {
		...
		swal('Sorry!', 'Please select sub-category field');
	}
}
			
			
function getCropnameDetail(name_id, variety_id){ 
	if(name_id!=''){  	
        $.ajax({
                ...
                if(responseArray.status == 1){
					...
					$.each(responseArray.variety_list,function(key,value){
						if(variety_id == value.id) {
							variety_list += '<option value='+value.id+' selected>'+value.variety+'</option>';
						} else {
		...
		swal('Sorry!', 'Please select crop name');
	}
}
...
function calculateExpectedHarverst (transplant_date){
	...
	if(cropname_id!=''){   
		...
		swal('Sorry!', 'Please provide crop name field');
	}	
}
...
function getlandDetail(farmer_id, land_id=null){
    if(farmer_id !=''){	
		$.ajax({
			...
			success:function(response) {
				...
				if(responseArray.status==1){
					...
					var land_list = '<option value="">Select Land Identification</option>';
		...
		swal('Sorry!', 'Please select farmer');
	}
} 
...
function getlandareaDetail(land_id){
    if(land_id!=''){
			...
			success:function(response) {
                ...
                if(responseArray.status == 1){
					...
					$.each(responseArray.landarea_list,function(key,value){
						...
						area_uom += '<option value='+value.measurement_unit+'>'+value.full_name+'</option>';
				    });
					$('#area_uom').html(area_uom);
				}
            },
			error:function(response){
				alert('Error!!! Try again');
		...
		swal('Sorry!', 'Please select land identification');
	}
}
