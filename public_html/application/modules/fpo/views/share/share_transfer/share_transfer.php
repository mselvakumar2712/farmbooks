    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content mt-3">
            ...
            <div class="animated fadeIn">
                    <form action="<?php echo base_url('fpo/share/postShareTransfer'); ?>" method="post" id="shareTransferForm" name="sentMessage" novalidate="novalidate" >
                    <div class="row">
							<div class="col-md-12">
								<div class="card">
									<div class="card-body">
                                        ...
                                        <div class="transfer_to_member" id="transfer_to_member" style="">
                                            ...
                                            <div class="row row-space">                
                                                <div class="form-group col-md-4">
                                                   <label for="inputAddress2">Transferrer Member Mobile Number <span class="text-danger">*</span></label>
                                                   <input type="hidden" class="form-control" name="transferor_id" id="transferor_id" placeholder="" required>
                                                ...
                                                <div class="form-group col-md-4">
                                                   <label for="inputAddress2">Transferee Member Mobile Number <span class="text-danger">*</span></label>
                                                   <input type="hidden" class="form-control" name="transferee_member_id" id="transferee_member_id" placeholder="" required>
                                                   <input type="hidden" class="form-control" name="transferee_id" id="transferee_id" placeholder="" required>
												   <input type="hidden" class="form-control" name="transferee_folio" id="transferee_folio" placeholder="" required>
                                                   <input type="hidden" class="form-control" name="transferee_member_type" id="transferee_member_type" placeholder="" required>
                                                   <input type="text" class="form-control numberOnly" id="transferee_member" maxlength="10" minlength="4" name="transferee_member" placeholder="Search member with mobile number" data-validation-required-message="Provide the search text" value="">
                                                   <div id="transferee_validate" class="with-errors text-danger"></div>
                                                </div>  
                                                <div class="form-group col-md-4">
                                                    <label for="inputAddress2">Date of Transfer <span class="text-danger">*</span></label>
                                                    <input type="date" class="form-control" id="transfer_date" name="transfer_date" placeholder="Date" value="<?php echo date("Y-m-d");?>" max="<?php echo date("Y-m-d");?>">
                                                </div>                                    
                                            </div>   
                                            ...
                                            <div class="row row-space" id="transferContent">
                                                ...
                                                <div class="form-group col-md-3">
                                                    ...
                                                    <input type="text" class="form-control numberOnly" name="distinctive_no_to" id="distinctive_no_to" placeholder="Distinctive No. To" data-validation-required-message="Provide distinctive no to share" readonly>
                                                    <div class="help-block with-errors text-danger"></div>
                                                </div>
                                                 <div class="form-group col-md-3">
                                                    <label for="inputAddress2">Folio Number <span class="text-danger">*</span></label>
                                                    <input type="text" class="form-control" id="folio_no" name="folio_number" placeholder="Folio Number" value="" readonly>
                                                </div>
                                            </div>                                                                                                   
                                        </div>
										...
										<div class="form-row" id="buttonContent">
								            <div class="form-group col-md-12 text-center">
											  <div id="success"></div>
												...
												<a href="<?php echo base_url('fpo/share/sharetransfer');?>" class="btn-fs btn btn-outline-dark"><i class="fa fa-close" aria-hidden="true"></i> Cancel</a>	
											 </div>											 
								        </div>
...
$("#transferor_member").focusout(function() {
	...
	var transferee_member = $("#transferee_member").val();
	if(transferor_member != '' && transferee_member != "") {
		if(transferor_member == transferee_member ) {
			var str = 'Transferrer member mobile number and Transferee member mobile number should not be same.';
			swal('', str);
			$(this).val(''); 
			return;
		}
...
function searchTransferorMember(search_member) {  
	$("#certificate_number").attr('readonly', false);
	$("#certificate_number").val('');
       $.ajax({
			url:"<?php echo base_url();?>fpo/share/verifyMember",
			type:"POST",
			data:search_member,
			dataType:"html",
			cache:false,			
			success:function(response) {                
                ...
                if(responseArray.status == 1) {                    
                    ...
                    $("#transferor_id").val(responseArray.member_data.id);   
                    $("#transferor_member_type").val(responseArray.member_data.member_type);     
                    //$("#transferor_folio").val(responseArray.member_data.folio_number); 
...
$("#transferee_member").focusout(function() {
    var transferee_member = $(this).val();
 	var transferor_member = $("#transferor_member").val();
	if(transferor_member != '' && transferee_member != "") {
		if(transferor_member == transferee_member ) {
			var str = 'Transferrer member mobile number and Transferee member mobile number should not be same.';
			swal('', str);
	...
	if(transferee_member != "") {       
        if(transferee_member.length == 10 && (transferee_member.charAt(0) == 6 || transferee_member.charAt(0) == 7 || transferee_member.charAt(0) == 8 || transferee_member.charAt(0) == 9)) {
            var search_member = {'mobilenumber':transferee_member, 'folio_number':""};
            searchTransfereeMember(search_member);
        } else if(transferee_member.length == 4) {
            var search_member = {'folio_number':transferee_member, 'mobilenumber':""};
            searchTransfereeMember(search_member);    
        }		
    }
});
    	
function searchTransfereeMember(search_member) {	
       $.ajax({
			url:"<?php echo base_url();?>fpo/share/verifyMember",
			type:"POST",
			data:search_member,
			...
			success:function(response) {                
                ...
                if(responseArray.status == 1) {    
                    ...
                    $("#transferee_member_id").val(responseArray.member_data.holder_id);
                    $("#transferee_id").val(responseArray.member_data.id);   
                    $("#transferee_member_type").val(responseArray.member_data.member_type);
					$("#transferee_folio").val(responseArray.member_data.folio_number);   
                    $("#transferee_validate").html("<div class='alert alert-success'>"+responseArray.message+"</div>");
				} else {
                    $("#transferee_validate").html("<div class='alert alert-danger'>"+responseArray.message+"</div>");
                    $("#transferee_member").focus();
                    $("#transferee_member").val('');
                } 
            }
         });            
}  
...
document.getElementById('certificate_number').oninput = function () {	
	...
	var certificate_info = {'holder_id':transferor_member_id, 'member_type':transferor_member_type, 'folio_number':folio_number};
		...
		$.ajax({
			url:"<?php echo base_url();?>fpo/share/getCertificateInfo",
			type:"POST",
			...
			success:function(response) {                
                ...
                if(responseArray.status == 1) {    
					...
					if(responseArray.certificateInfo[0].certificate_num != ""){
						$("#certificate_number").attr('readonly', true);
					} else {
						$("#certificate_number").attr('readonly', false);
					}
