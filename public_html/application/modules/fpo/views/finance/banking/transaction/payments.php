      ...
      <div class="container-fluid pl-0 pr-0">
                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            ...
                            <li><a href="<?php echo base_url('fpo/finance/payments');?>"class="active">Payment</a></li>
                        </ol>
                    </div>
                </div>
            </div>
        ...
        <div class="content mt-3">
            ...
            <div class="animated fadeIn">
               <form action="<?php echo base_url('fpo/finance/postPayment')?>" method="post" id="paymentform" name="sentMessage" novalidate="novalidate" >
                  <div class="row">
							<div class="col-md-12">
								<div class="card">
									<div class="card-body">
										<div class="container-fluid">                                            
										  <div class="row ">
												<div class="form-group col-md-4">
													...
													<input class="form-control" type="date" autofocus min="<?php echo $formation_date;?>" id="payment_date" name="payment_date" required data-validation-required-message="Select payment date" value="<?php echo date("Y-m-d"); ?>" max="<?php echo date("Y-m-d"); ?>">
												...
												<div class="form-group col-md-4">
													...
													<input type="text" class="form-control" id="voucher_no" name="voucher_no" placeholder="Voucher No" required data-validation-required-message="Enter the voucher no" value="<?php echo $voucher_no; ?>" readonly>
													<p class="help-block text-danger"></p>
												</div>							
                                          ...
                                          <tr id="row0">                                               
                                             <td width="25%">
                                                <select class="form-control" style="width:350px;" id="cost_center_0" name="payment[0][cost_center]" required data-validation-required-message="Select any paid from">
                                                </select>
                                                <p id="ledgerBalance_0" class="text-danger ledgerBalance mt-2 mb-0 pb-0 pt-0"></p>
                                                <span class="help-block text-danger"></span>
                                             </td>
                                             <td width="25%">
                                                <select class="form-control" style="width:350px;" id="payment_from_0" name="payment[0][payment_from]" required data-validation-required-message="Select any paid from">
                                                </select>
                                                <p id="accountBalance_0" class="text-danger accountBalance mt-2 mb-0 pb-0 pt-0"></p>
                                                <span class="help-block text-danger"></span>
                                             </td>
                                             <td width="15%">
                                                <input type="text" name="payment[0][amount]" class="form-control numberOnly text-right" id="amount_0" maxlength="6" readonly/>
                                                <span class="help-block text-danger"></span>
                                             </td>  
                                             <td width="10%">
...
$(document).ready(function() {
    ...
    $(document).on('click', '#sendMessageButton', function(){ 
		...
		if($('#cost_center_0').val() == '' || $('#payment_from_0').val() == '' || $('#amount_0').val() == ''){
			validate = false;
		}
		if(validate){
			return true;
...
$('#amount_0').focusout(function (e){
   ...
   var received_from = $('#cost_center_0').val();
...
function getAllSupplierByFPO(payment_to) { 
    $.ajax({
        url:"<?php echo base_url();?>fpo/finance/getAllSupplierByFPO",
        type:"GET",
        data:"",
        dataType:"html",
        cache:false,			
      ...
      dataType:"html",        
        ...
        success:function(response) {
            ...
            if(responseArray.ledger_balance != null) {
                ...
                $('#'+ledgerBalance).html('<strong> Balance: <i class="fa fa-inr" aria-hidden="true"></i>'+(temp_totalLedgerBalance[ledgerValue] >=0 ? temp_totalLedgerBalance[ledgerValue]+' Cr':-(temp_totalLedgerBalance[ledgerValue])+' Dr')+'</strong>');
            } else {
                $('#'+ledgerBalance).html('<strong> Balance: <i class="fa fa-inr" aria-hidden="true"></i>0.00 Cr</strong>');
            }   
        },
        error:function(response){
            console.log(response);
	...
	$.ajax({
      ...
      success:function(response) {
         ...
         if(responseArray.account_balance != null) {
            ...
            $('#'+accountBalance).html('<strong> Balance: <i class="fa fa-inr" aria-hidden="true"></i>'+(temp_totalBalance[accId] >=0 ? temp_totalBalance[accId]+' Cr':-(temp_totalBalance[accId])+' Dr')+'</strong>');
         } else {
...
function verifyBalanceAmount(amountValue, amountInputId, accountId, updateBalanceId, received_from, updateLedgerBalance){    
	...
	//alert(Number(amountValue) +':::'+ Number(-(temp_totalBalance[accountId[0]])));
   if(Number(amountValue) > Number(-(temp_totalBalance[accountId[0]]))) {
      ...
      $('#'+updateBalanceId).html('<strong> Balance: <i class="fa fa-inr" aria-hidden="true"></i>'+(temp_totalBalance[accountId[0]] >=0 ? temp_totalBalance[accountId[0]]+' Cr':-(temp_totalBalance[accountId[0]])+' Dr')+'</strong>');
      $('#'+updateLedgerBalance).html('<strong> Balance: <i class="fa fa-inr" aria-hidden="true"></i>'+(temp_totalLedgerBalance[ledgerId[0]] >=0 ? temp_totalLedgerBalance[ledgerId[0]]+' Cr':-(temp_totalLedgerBalance[ledgerId[0]])+' Dr')+'</strong>');
   }
}
</script>
</body>
