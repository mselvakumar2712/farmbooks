...
$(document).ready(function() {
    $.ajax({
        url:"<?php echo base_url();?>fpo/farmer/farmimplement_list",
        type:"GET",
        dataType:"html",
        cache:false,			
              ...
              var count=1;
                $.each(responseArray.farmimplement_list,function(key,value){
                    ...
                    if(value.farm_implements == 1) {
                        farmimplement_list += '<tr><td>'+value.profile_name+'</td><td>'+value.implement_name+'</td><td>'+implement_make+'</td><td>'+implement_model+'</td><td>'+value.year+'</td><td>'+available_for_hire+'</td><td>'+purchase_by_loan+'</td><td>'+insurance_coverage+'</td><td><a href="<?php echo base_url("fpo/farmer/view_farmimplement?id='+value.id+'");?>" class="btn btn-success btn-sm" title="Edit"><i class="fa fa-edit" aria-hidden="true" title="Edit"></i></a><a href="<?php echo base_url("fpo/farmer/view_farmimplement?id='+value.id+'");?>" class="btn btn-warning mt-2 btn-sm" title="View"><i class="fa fa-eye" aria-hidden="true" title="View"></i></a></td></tr>';
                    }
                });
                $('#farm_implements_list').html(farmimplement_list);
				
				if(responseArray.fpo_location){       
                    stateCode = responseArray.fpo_location['state'];                    
					districtCode = responseArray.fpo_location['district'];                    
					blockCode = responseArray.fpo_location['block'];                    
					panchayatCode = responseArray.fpo_location['panchayat'];                    
					villageCode = responseArray.fpo_location['village'];
...
function loadFarmers(stateCode, districtCode, blockCode, panchayatCode, villageCode){
    if(stateCode != "" && districtCode != "" && blockCode != "") {
        ...
        $.ajax({
			url:"<?php echo base_url();?>fpo/farmer/farmImplementsByLocation",
			type:"POST",
			data:formedCode,
			dataType:"html",
			cache:false,			
		   ...
		   responseArray=$.parseJSON(response);
			   if(responseArray.status == 1){
					var farmimplement_list = "";
					if(responseArray.farmimplement_list.length != 0) {						
						$.each(responseArray.farmimplement_list,function(key,value){
							if(value.available_for_hire == 1) {
								var available_for_hire = "Yes";                   
							} else {
					   ...
					   farmimplement_list += '<tr><td colspan="9" class="text-center">No records found</td></tr>';
					}
