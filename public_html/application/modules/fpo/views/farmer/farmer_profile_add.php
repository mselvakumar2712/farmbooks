              		...
              		<div id="step-1">
							...
							<div class="form-group col-md-12 mt-1">
								<div class="form-group col-md-6 mt-2">
									...
									<input type="text" class="form-control lettersonly" maxlength="100" id="farmer_profile_name" name="farmer_profile_name" placeholder="Name of The Farmer" required>
									 <div class="help-block with-errors text-danger"></div>
								</div>
								<div class="form-group col-md-6 mt-2">
									<label for="">Alias Name</label>
									<input type="text" class="form-control lettersonly" maxlength="100" id="farmer_alias_name" name="farmer_alias_name" placeholder="Alias Name">
								</div>
								</div>
								<div class="form-group col-md-12 mt-1">
								<div class="form-group col-md-6">
									<label for="inputAddress2">Father Name/Spouse Name <span class="text-danger">*</span></label>
									<input type="text" class="form-control lettersonly" id="farmer_father_spouse_name" name="farmer_father_spouse_name" maxlength="100" placeholder="Father Name/Spouse Name" required>
									 <div class="help-block with-errors text-danger"></div>
								</div>
								<div class="form-group col-md-6">
...
//                    LoadCertifiedProduct();
         ...
         $('form').submit(function(e){
            ...
            if(elmErr && elmErr.length > 0){
               ...
               $.ajax({
                  url:"<?php echo base_url();?>fpo/farmer/postfarmer",
                  type:"POST",
                  data:farmerData,
                  dataType:"html",
                  cache:false,
...
function getLiveStockType(value) {
        $.ajax({
			url:"<?php echo base_url();?>fpo/farmer/getLiveStocks/"+value,
			type:"GET",
			data:"",
			dataType:"html",
            cache:false,
...
function LoadCertifiedProduct() {
        $.ajax({
			url:"<?php echo base_url();?>fpo/farmer/getProductList",
			type:"GET",
			data:"",
			dataType:"html",
            cache:false,
...
$('select[id="live_stocks_type"]').on('change', function() {
    ...
    $.ajax({
			url:"<?php echo base_url();?>fpo/farmer/getLiveStocks/"+live_stocks_type,
			type:"GET",
			data:"",
			dataType:"html",
            cache:false,
...
$('select[id="live_stocks_name"]').on('change', function() {
    ...
    $.ajax({
			url:"<?php echo base_url();?>fpo/farmer/getLiveStockVariety/"+live_stocks_name,
			type:"GET",
			data:"",
			dataType:"html",
            cache:false,
...
function getAreaUOM() {
        $.ajax({
			url:"<?php echo base_url();?>fpo/farmer/getAreaUOM",
			type:"GET",
			data:"",
			dataType:"html",
            cache:false,
...
$("#farmerNoMobile").change(function(event){
    if(this.checked){
        ...
        var randomCode = Math.floor(2000000000 + (Math.random() * 2345));
        var generatedCode = parseInt(farmer_block)+parseInt(randomCode);
        generatedCode = generatedCode.toString();
        //alert(farmer_block+'---'+generatedCode);
