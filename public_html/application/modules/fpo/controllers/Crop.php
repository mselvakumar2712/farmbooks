...
class Crop extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['crop_list'] = $this->Crop_Model->cropList();
    ...
    public function viewcrop($crop_id)
    {
        $data['page'] = 'Crop_Farmer_Mapping';
        $data['page_module'] = 'crop';
        $data['page_title'] = 'View Crop';
        ...
        $croplist = $this->Crop_Model->cropByID($crop_id);
        $data['croplist'] = $croplist[0];
    ...
    public function edit_crop($crop_id)
    {
        ...
        $data['beforeMonth'] = date("Y-m-d", strtotime("-1 month"));
        $data['afterMonth'] = date("Y-m-d", strtotime("+3 month"));
        $data['croplist'] = $this->Crop_Model->cropId($crop_id);
        //$data['croplist'] = $croplist;
    ...
    public function postcrop()
    {
        ...
        } elseif ($this->Crop_Model->addcrop()) {
    ...
    public function updatecrop($crop_id)
    {
        if ($this->cropValidator() == 0) {
            ...
            redirect('fpo/crop');
        } elseif ($this->Crop_Model->updatecrop($crop_id)) {
