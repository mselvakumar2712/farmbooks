...
class Farmer extends CI_Controller
{
    ...
    public function profilelist()
    {
        ...
        $data['page_module'] = 'farmer';
        $data['state_list'] = $this->Fpo_Model->getStateList();
        $fpo_location = $this->Fpo_Model->getBlockByFPO();
        $_SERVER["REQUEST_METHOD"] = "POST";
        $_POST['stateCode'] = $fpo_location->state;
        $_POST['districtCode'] = $fpo_location->district;
        $_POST['blockCode'] = $fpo_location->block;
        $_POST['panchayatCode'] = $fpo_location->panchayat;
        $_POST['villageCode'] = $fpo_location->village;
        $farmer_list = $this->Farmer_Model->farmersByLocation();
        $data['farmer_list'] = $farmer_list;
        //echo "<pre>";print_r($farmer_list);die;
    ...
    {
        $fpo_location = $this->Fpo_Model->getBlockByFPO();
        $farmer_list = $this->Farmer_Model->farmersProfileListByBlock($fpo_location->block);
        $response = array("status" => 1, "farmer_list" => $farmer_list, 'fpo_location' => $fpo_location);
    ...
    {
        $land_list = $this->Farmer_Model->landByLocation();
    ...
    {
        $farmimplement_list = $this->Farmer_Model->farmImplementsByLocation();
    ...
    {
        $livestock_list = $this->Farmer_Model->liveStocksByLocation();
    ...
    public function land_list()
    {
        ...
        //echo "<pre>";
        //print_r($fpo_location);
        //print_r($land_list);die;
        $response = array("status" => 1, "land_list" => $land_list, "fpo_location" => $fpo_location);
        echo json_encode($response);
    }
    ...
    public function farmimplement_list()
    {
        ...
        $response = array("status" => 1, "farmimplement_list" => $farmimplement_list, "fpo_location" => $fpo_location);
        echo json_encode($response);
    }
    ...
    public function livestock_list()
    {
        ...
        $response = array("status" => 1, "livestock_list" => $livestock_list, "fpo_location" => $fpo_location);
        echo json_encode($response);
    }
    ...
    public function postFarmer()
    {
        if ($this->Login_Model->mobileNumberExists($this->input->post('farmer_mobile_num')) > 0) {
            ...
            if ($this->Farmer_Model->addFarmer()) {
                $response = array("status" => 1, "message" => "Farmer information successfully added !!!");
            } else {
    ...
    public function updateFarmer($farmer_id)
    {
        if ($this->Farmer_Model->updateFarmer($farmer_id)) {
            ...
            redirect('fpo/farmer/profilelist');
        } else {
            $this->session->set_flashdata('warning', 'Profile image is not uploaded');
            redirect('fpo/farmer/editfarmer/'.$farmer_id);
        }
        //echo json_encode($response);
    }
    ...
    {
        $farmImplement_make    = $this->Farmer_Model->getFarmImplementsMake($name_value);
        $response = array("status" => 1, "farmImplement_make" => $farmImplement_make);
        echo json_encode($response);
    }

    public function getFarmImplementsModel($imp_id, $name_value)
    {
        $farmImplement_model    = $this->Farmer_Model->getFarmImplementsModel($imp_id, $name_value);
        $response = array("status" => 1, "farmImplement_model" => $farmImplement_model);
        echo json_encode($response);
    }
