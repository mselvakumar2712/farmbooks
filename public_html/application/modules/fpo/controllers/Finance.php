...
class Finance extends CI_Controller
{
    ...
    public function reconcilebankaccount()
    {
        ...
        $data['page_module'] = 'finance';
        $fromValue = ($this->input->post('tax_from'))?$this->input->post('tax_from'):date('Y-m-01');
        $toValue = ($this->input->post('tax_to'))?$this->input->post('tax_to'):date('Y-m-d');
        $data['banks'] = $this->Finance_Model->getAllBankByFPO();
    ...
    public function searchreconsile()
    {
        $data['page'] = 'Transaction';
        $data['page_title'] = 'Reconcile Bank Account';
        $data['page_module'] = 'finance';
        ...
        $fromValue = ($this->input->post('tax_from'))?$this->input->post('tax_from'):date('Y-m-01');
        $toValue = ($this->input->post('tax_to'))?$this->input->post('tax_to'):date('Y-m-d');
        $account = ($this->input->post('account'))?$this->input->post('account'):date('Y-m-01');
        $data['banks'] = $this->Finance_Model->getAllBankByFPO();
        $data['transactions'] = $this->Finance_Model->getSelectedBankTransfer($fpo_id, $fromValue, $toValue, $selectedCode,$account);
        //echo "<pre>";print_r($data);die;
        $data['fromValue'] = $fromValue;
        $data['toValue'] = $toValue;
    ...
    public function taxinquiry()
    {
        ...
        $fromValue = ($this->input->post('tax_from'))?$this->input->post('tax_from'):date('Y-m-01');
        $toValue = ($this->input->post('tax_to'))?$this->input->post('tax_to'):date('Y-m-d');
        $selectedCode = '3030101';
        $taxes = $this->Finance_Model->getTaxInOut($this->session->userdata('user_id'), $fromValue, $toValue, $selectedCode);
        $outputGST = $inputGST = array();
        $GSTout = $GSTin = 0;
        if(!empty($taxes)){
          foreach($taxes as $key => $value){
            if($value->cgst > 0 || $value->sgst > 0 || $value->igst > 0){
              if($value->account_code == '303010101'){
                  $gst = $value->cgst_percent*2;
                  if(!isset($inputGST[$gst]['cgst']))
                    $inputGST[$gst]['cgst'] = 0;
                  $inputGST[$gst]['cgst'] += $value->cgst;
                  $GSTin += $value->cgst;
              }elseif($value->account_code == '303010102'){
                  $gst = $value->igst_percent;
                  if(!isset($inputGST[$gst]['igst']))
                    $inputGST[$gst]['igst'] = 0;
                  $inputGST[$gst]['igst'] += $value->igst;
                  $GSTin += $value->igst;
              }elseif($value->account_code == '303010103'){
                  $gst = $value->sgst_percent*2;
                  if(!isset($inputGST[$gst]['sgst']))
                    $inputGST[$gst]['sgst'] = 0;
                  $inputGST[$gst]['sgst'] += $value->sgst;
                  $GSTin += $value->sgst;
              }elseif($value->account_code == '303010104'){
                  $gst = $value->cess_percent;
                  if(!isset($inputGST[$gst]['cess']))
                    $inputGST[$gst]['cess'] = 0;
                  $inputGST[$gst]['cess'] += $value->cess;
                  $GSTin += $value->cess;
              }elseif($value->account_code == '303010105'){
                  $gst = $value->cgst_percent*2;
                  if(!isset($outputGST[$gst]['cgst']))
                    $outputGST[$gst]['cgst'] = 0;
                  $outputGST[$gst]['cgst'] += $value->cgst;
                  $GSTout += $value->cgst;
              }elseif($value->account_code == '303010106'){
                  $gst = $value->igst_percent;
                  if(!isset($outputGST[$gst]['igst']))
                    $outputGST[$gst]['igst'] = 0;
                  $outputGST[$gst]['igst'] += $value->igst;
                  $GSTout += $value->igst;
              }elseif($value->account_code == '303010107'){
                  $gst = $value->sgst_percent*2;
                  if(!isset($outputGST[$gst]['sgst']))
                    $outputGST[$gst]['sgst'] = 0;
                  $outputGST[$gst]['sgst'] += $value->sgst;
                  $GSTout += $value->sgst;
              }elseif($value->account_code == '303010108'){
                  $gst = $value->cess_percent;
                  if(!isset($outputGST[$gst]['cess']))
                    $outputGST[$gst]['cess'] = 0;
                  $outputGST[$gst]['cess'] += $value->cess;
                  $GSTout += $value->cess;
              }
            }
          }
        ...
        $data['fromValue'] = $fromValue;
        $data['toValue'] = $toValue;
        $data['outputGST'] = $outputGST;
        $data['inputGST'] = $inputGST;
        $data['GSTout'] = $GSTout;
        $data['GSTin'] = $GSTin;
        //echo "<pre>";print_r($data);die;
    ...
    public function trialbalance()
    {
        ...
        if ($month < '04') {
            $data['fromValue'] = date('Y-04-01', strtotime('-1 year'));
        } else {
            $data['fromValue'] = date('Y-04-01');
        }
    ...
    public function postTrialBalance()
    {
        ...
        if ($purchase_stock->total_qty != null && $purchase_stock->total_amount != null) {
            ...
            $data['closingStock'] = round($purchaseValue,2)*$averageQty;
        } else {
            $data['closingStock'] = 0;
        }
        /* Current year closing stock calculations end */
        ...
        if ($lastYear_purchase_stock->total_qty != null && $lastYear_purchase_stock->total_amount != null) {
            ...
            $data['openingStock'] = -(round($purchaseValue,2)*$averageQty);
        } else {
            $data['openingStock'] = 0;
        }
    ...
    public function profitAndLossReports()
    {
        ...
        /* Current year closing stock calculations end */
        $purchase_stock = $this->Finance_Model->getInventoryStockUpto($this->session->userdata('user_id'),  $toValue, 1);
        $sales_stock = $this->Finance_Model->getInventoryStockUpto($this->session->userdata('user_id'),  $toValue, 2);

        if (!empty($purchase_stock)) {
            $currentStock = 0;
            $salesstock = array();
            if (!empty($sales_stock)) {
                foreach ($sales_stock as $stockvalue) {
                    $salesstock[$stockvalue->stock_id] = $stockvalue->total_qty;
                }
            }
            foreach ($purchase_stock as $stockvalue) {
                $stock_id = $stockvalue->stock_id;
                $avg_amount = ($stockvalue->total_amount)/$stockvalue->total_qty;
                if(array_key_exists($stock_id, $salesstock)){
                    $balanceStock = $stockvalue->total_qty+$salesstock[$stock_id];
                    $currentStock += ($balanceStock)*$avg_amount;;
                }else{
                    $currentStock += ($stockvalue->total_qty)*$avg_amount;
                }

            }
            $data['closingStock'] = round($currentStock,2);
        } else {
            $data['closingStock'] = 0;
        }
        //echo "<br>With Current Closing:".$data['closingStock'];die;
        /* Current year closing stock calculations end */
        ...
        if ($month < '04') {
          ...
          $lastYearFromValue = date('Y-04-01', strtotime("-1 year"));
          $lastYearToValue = date('Y-03-31');
        }

        $lastYear_purchase_stock = $this->Finance_Model->getAllInventorySum($this->session->userdata('user_id'), $lastYearFromValue, $lastYearToValue, 1);
        $lastYear_sales_stock = $this->Finance_Model->getAllInventorySum($this->session->userdata('user_id'), $lastYearFromValue, $lastYearToValue, 2);
        //echo "<pre>"; print_r($lastYear_purchase_stock);print_r($lastYear_sales_stock);die;
        if ($lastYear_purchase_stock->total_qty != null && $lastYear_purchase_stock->total_amount != null) {
            $averageQty = $lastYear_purchase_stock->total_qty + $lastYear_sales_stock->total_qty;
            $purchaseValue = $lastYear_purchase_stock->total_amount / $lastYear_purchase_stock->total_qty;
            $data['openingStock'] = -(round($purchaseValue,2)*$averageQty);
    ...
    {
        if($this->input->post('accountCode') == '30302' || $this->input->post('accountCode') =='40202'){
          $glCategory = $this->Finance_Model->getSalesPurchaseNodeByCode($this->session->userdata('user_id'));
        }else{
...
//        $this->load->view('finance/banking/maintenance/bank_accounts', $data);
    ...
    public function postBankAccountValidator()
    {
...
//    }
    ...
    public function getCustomername()
    {
        ...
        } elseif (!empty($get_supplier) && !empty($get_supplier[0]->supp_reference) && empty($get_customer[0]->reference)) {
    ...
    public function salesinvoicepdf($voucher_no, $inv_no, $customer)
    {
        ...
        for ($i=0;$i<count($salesTransaction);$i++) {
            ...
            } elseif ($salesTransaction[$i]->stock_group_id == 2) {
    ...
    //values in words
    public function get_numberTowords($number)
    {
        $no = round($number);
