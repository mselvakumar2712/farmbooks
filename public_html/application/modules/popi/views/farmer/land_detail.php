...
<!--
								    <a href="<?php echo base_url('popi/farmer/addland');?>">
...
<script type="text/javascript">
      $(document).ready(function() {
			$.ajax({
				url: "<?php echo base_url();?>popi/farmer/land_list",
				type: "GET",
				dataType:"html",
				cache:false,			
				...
				if(responseArray.status == 1){
                    ...
                    $.each(responseArray.land_list,function(key,value){
                        ...
                        if(value.land_holdings == 1) {                           
                            land_list += '<tr><td>'+value.profile_name+'</td><td>'+ownership_type+'</td><td>'+value.name+'</td><td>'+land_type+'</td><td>'+address+'</td><td><a href="<?php echo base_url("popi/farmer/viewland/'+value.id+'");?>" class="btn btn-success btn-sm" title="Edit"><i class="fa fa-edit" aria-hidden="true" title="Edit"></i></a><a href="<?php echo base_url("popi/farmer/viewland/'+value.id+'");?>" class="btn btn-warning mt-2 btn-sm" title="View"><i class="fa fa-eye" aria-hidden="true" title="View"></i></a></td></tr>';
                        }
                    });
