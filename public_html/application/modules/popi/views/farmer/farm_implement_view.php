...
<!--									<a href="#" id="" class="del btn btn-danger">Delete</a>-->
									<a href="<?php echo base_url('popi/farmer/farmimplementlist');?>" id="ok" class="btn btn-fs btn-outline-dark"><i class="fa fa-arrow-circle-left"></i> Back</a>	
									<a href="<?php echo base_url('popi/farmer/farmimplementlist');?>" id="cancel" class="btn btn-fs btn-outline-dark" style="display:none;"><i class="fa fa-close" aria-hidden="true"></i> Cancel</a>								
								</div>
						       </form>
						     </div>
					      </div>
...
//sweetalert with delete function
			$('a.del').click(function() {
                ...
                swal({
                 ...
                 if (result.value) {          
                    ...
                    $.ajax({
                      url: "<?php echo base_url();?>popi/farmer/deleteFarmImplement/"+farmimplement_id,
                      type: "POST",
                      data: {
                      ...
                      success: function() {        
                         setTimeout(function() {
                          window.location.replace("<?php echo base_url('popi/farmer/farmimplementlist')?>");
                         }, 1000);
                      },
                      error: function() {
   ...
   $(document).ready(function(){	
        ...
        $('input').on('click', function() {
            ...
            // jQuery method
			$.ajax({
			url:"<?php echo base_url();?>popi/farmer/editfarmimplement/"+farmimplement_id,
			type:"GET",
			dataType:"html",
			cache:false,			
			success:function(response){		  
                ...
                if(responseArray.status == 1){
                    ...
                    if(responseArray.farmimplement_list[0].farm_implements == 1) {
                        ...
                        var href_value="<?php echo base_url('popi/farmer/add_farmimplement?id=');?>"+responseArray.farmimplement_list[0].farmer_id;
...
$('form').submit(function(e){
        ...
        $.ajax({
            url:"<?php echo base_url();?>popi/farmer/updatefarmimplement/"+farm_implement_id,
            type:"POST",
            data:farmerdata,
            dataType:"html",
            cache:false,      
