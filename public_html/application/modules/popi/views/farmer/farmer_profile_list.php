                ...
                <div class="page-header float-right">
                    <div class="page-title">
                        <ol class="breadcrumb text-right">
                            ...
                            <li><a class="active" href="<?php echo base_url('popi/farmer/profilelist');?>">Farmer Profile </a></li>
                        </ol>
                    </div>
                </div>
            </div>
                               ...
                               <div class="row">
                                   <div class="form-group col-md-4">
                                    ...
                                    <div class="form-group col-md-6 mt-4" style="text-align:right;" >
                                        <a href="<?php echo base_url('popi/farmer/addfarmer');?>">
...
var villageCode = 0;    
      ...
      $(document).ready(function() {
			$.ajax({
				url: "<?php echo base_url();?>popi/farmer",
				type: "POST",
				dataType:"html",
				cache:false,			
               ...
               if(responseArray.farmer_list.length != 0) {
                  $.each(responseArray.farmer_list,function(key,value){
                     ...
                     farmer_list += '<tr><td>'+value.profile_name+'</td><td>'+dateFormat+' / '+value.age+'</td><td>'+value.mobile+'</td><td><?php if('+value.aadhar_no+'){echo 'Active';}else{echo 'In Active';}?></td><td><a href="<?php echo base_url("popi/farmer/editfarmer/'+value.id+'");?>" class="btn btn-success btn-sm" title="Edit"><i class="fa fa-edit" aria-hidden="true" title="Edit"></i></a><a href="<?php echo base_url("popi/farmer/viewfarmer/'+value.id+'");?>" class="btn btn-warning ml-1 btn-sm" title="View"><i class="fa fa-eye" aria-hidden="true" title="View"></i></a></td></tr>';
                  });
               } else {
...
function getDistrictByState(stateCode){
    $.ajax({
			url:"<?php echo base_url();?>popi/farmer/getDistrictByState/"+stateCode,
			type:"GET",
			data:"",
			dataType:"html",
            cache:false,			
...
function getBlockByDistrict(districtCode){
    $.ajax({
			url:"<?php echo base_url();?>popi/farmer/getBlocksByDistrict/"+districtCode,
			type:"GET",
			data:"",
			dataType:"html",
            cache:false,			
...
function loadFarmers(stateCode, districtCode, blockCode, panchayatCode, villageCode) {
    if(stateCode != "" && districtCode != "" && blockCode != "") {
        ...
        $.ajax({
				url:"<?php echo base_url();?>popi/farmer/farmersByLocation",
				type:"POST",
                data:formedCode,
				dataType:"html",
				cache:false,			
               ...
               var count=1;
                   if(responseArray.farmer_list.length != 0) {
                       $.each(responseArray.farmer_list,function(key,value){
                           var sno=count++;
                           var dateFormat = dateFormatChange(value.dob);        
                           farmer_list += '<tr><td>'+value.profile_name+'</td><td>'+dateFormat+' / '+value.age+'</td><td>'+value.mobile+'</td><td><?php if('+value.aadhar_no+'){echo 'Active';}else{echo 'In Active';}?></td><td><a href="<?php echo base_url("fpo/farmer/editfarmer/'+value.id+'"); ?>" class="btn btn-success btn-sm" title="Edit"><i class="fa fa-edit" aria-hidden="true" title="Edit"></i></a><a href="<?php echo base_url("fpo/farmer/viewfarmer/'+value.id+'");?>" class="btn btn-warning btn-sm" title="View"><i class="fa fa-eye" aria-hidden="true" title="View"></i></a></td></tr>';
                       });
