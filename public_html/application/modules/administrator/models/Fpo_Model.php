...
/** FPO Starts **/
    function fpoList() {
        $this->db->select('trv_fpo.id,trv_fpo.user_id, UPPER(trv_fpo.producer_organisation_name) as producer_organisation_name, trv_fpo.mobile, trv_fpo.constitution, trv_fpo.contact_person, trv_fpo.status, trv_popi.institution_name');        
      ...
      function addfpo() {
          ...
          $user_id=$this->db->select('trv_popi.user_id')->where(array('trv_popi.id' =>$this->input->post('popi_name')))->from('trv_popi')->get()->row()->user_id;
    ...
    function updatefpo($fpo_id) {
        $user_id=$this->db->select('trv_popi.user_id')->where(array('trv_popi.user_id' =>$this->input->post('popi_id')))->from('trv_popi')->get()->row()->user_id;
        $fpo_details = array(
            'popi_id'                       =>$user_id,
			'producer_organisation_name'    => $this->input->post('producer_organisation_name'),
            'pin_code'                      => $this->input->post('pin_code'),
