...
/** Crop Management Common drop down API functions **/    
    ...
    function nutrientDropdownList() {
        $this->db->select('trv_nutrient_master.id, trv_nutrient_master.nutrient_type, trv_nutrient_master.name, trv_nutrient_master.brand_name');
    ...
    function fertilizerDropdownList() {
        $this->db->select('trv_fertilizer_master.id, trv_fertilizer_master.fertilizer_type, trv_fertilizer_master.name, trv_fertilizer_master.brand_name');
    ...
    function pesticideDropdownList() {
        $this->db->select('trv_pesticide_master.id, trv_pesticide_master.pesticide_type, trv_pesticide_master.name, trv_pesticide_master.brand_name');
    ...
    function updatecropharvest($crop_id) {  
        ...
        for($i=0;$i<count($output_product);$i++){
		    ...
		    $this->db->update('trv_crop_harvest', $crop_details, array('id' => $harvestid[$i]));
			 $cost_details = array(	
            'crop_id'               => $this->input->post('variety'),
            'post_harvesting_cost' => $this->input->post('harvest_cost'),
            'farmer_id'             => $this->input->post('farmer_id'),
            //'image'                 => ($img_data) ? $img_data['file_name']:"",
...
/** Post Crop harvest End **/    
  ...
  function subcategoryByCategory($category_id) {
	 ...
	 function expensesssList( $expenses_id ) {
        ...
        $this->db->from('trv_update_expense');
		  $this->db->where(array('trv_update_expense.crop_id' =>  $expenses_id,));
        return $this->db->get()->result();
    }
