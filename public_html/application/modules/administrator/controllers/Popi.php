...
/** View FPG profile related view by using the given functions **/
   public function index() {        
	...
	public function addpopi() {
      ...
      $data['page_module'] = 'profile';		
      $financial_year = date("m") > 3?date("Y-04-01"):date("Y-04-01", strtotime('-1 year'));//year-month-date
		$data['financial_year']= $financial_year;
      $date_formation = date("Y-m-d");
    ...
    public function viewpopi($popi_id) {
      ...
      if(!empty($popi_list)){         
         $data['taluk_info'] = $this->Login_Model->getTalukByPinCode($popi_list->pin_code);
         $data['block_info'] = $this->Login_Model->getBlockByDistCode($popi_list->district);
         $data['panchayat_info'] = $this->Login_Model->getPanchayatByBlockcode($popi_list->block);
         $data['village_info'] = $this->Login_Model->getVillageByPanchayatcode($popi_list->panchayat);
      }
      $data['popi_info']= $popi_list;
