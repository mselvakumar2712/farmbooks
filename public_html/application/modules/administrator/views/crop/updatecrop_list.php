                        ...
                        <div class="card-body">	
                           <div class="container">
										<div class="" style="text-align:right;margin-bottom:20px;" >
											<a href="<?php echo base_url('administrator/Updatecrop/add_updatecrop');?>">
...
$(document).ready(function() {
    //var crop_id =<?php //echo $_REQUEST['crop_id']?>;
            //CROP list API CALL
				$.ajax({
						url:"<?php echo base_url();?>administrator/updatecrop/updatecroplist",
						type:"GET",
						data:"",
						dataType:"html",
						cache:false,			
						...
						console.log(responseArray.update_crop);
						 if(responseArray.status == 1){
							...
							$.each(responseArray.update_crop,function(key,value){
                                ...
                                /* var brand_name=[];
                                if(value.brand_name != "" && value.brand_name != null) {
                                    console.log(value.brand_name);
                                    $.each(value.brand_name,function(keys,values){
                                        brand_name.push(values.name);
                                    });
                                } */
                                
							 updatecrop_list += '<tr><td>'+value.profile_name+'</td><td>'+value.variety_name+'</td><td>'+update_type+'</td><td>'+brandname+'</td><td>'+value.process_date+'</td><td>'+value.dosage+'</td><td>'+value.qty+'/'+value.fullname+'</td><td class="text-right">'+value.process_cost+'</td><td class="text-center" ><a href="<?php echo base_url("administrator/updatecrop/edit_updatecrop?id='+value.id+'");?>" class="btn btn-success btn-sm" title="Edit"><i class="fa fa-edit" aria-hidden="true" title="Edit"></i></a><a href="<?php echo base_url("administrator/updatecrop/view_updatecrop?id='+value.id+'");?>" class="btn btn-warning btn-sm mt-2" title="View"><i class="fa fa-eye" aria-hidden="true" title="View"></i></a></td></tr>';
							});
