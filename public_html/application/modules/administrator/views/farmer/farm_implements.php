...
<!--
								<a href="<?php //echo base_url('administrator/farmer/addlivestock');?>">
...
$(document).ready(function() {
    $.ajax({
        url:"<?php echo base_url();?>administrator/farmer/farmimplement_list",
        type:"GET",
        dataType:"html",
        cache:false,			
              ...
              var count=1;
                $.each(responseArray.farmimplement_list,function(key,value){
                    ...
                    if(value.farm_implements == 1) {
                        farmimplement_list += '<tr><td>'+value.profile_name+'</td><td>'+value.implement_name+'</td><td>'+implement_make+'</td><td>'+implement_model+'</td><td>'+value.year+'</td><td>'+available_for_hire+'</td><td>'+purchase_by_loan+'</td><td>'+insurance_coverage+'</td><td><a href="<?php echo base_url("administrator/farmer/view_farmimplement?id='+value.id+'");?>" class="btn btn-success btn-sm" title="Edit"><i class="fa fa-edit" aria-hidden="true" title="Edit"></i></a><a href="<?php echo base_url("administrator/farmer/view_farmimplement?id='+value.id+'");?>" class="btn btn-warning btn-sm mt-2" title="View"><i class="fa fa-eye" aria-hidden="true" title="View"></i></a></td></tr>';
                    }
                });
