...
/** FPO Starts **/
	...
	function salespersoninactivelist() {  
       $fpoid=$this->session->userdata('user_id');	 
        $this->db->select('fpo_salesman.*');
        $this->db->from('fpo_salesman');
	...
	function updatesalesperson($salesman_code) {
        $salespersons = array(
           'fpo_id'                  => $this->session->userdata('user_id'),		  
	        'salesman_name'				 => $this->input->post('sales_person_name'),
            'salesman_phone'          => $this->input->post('mobile_num'),
   ...
   function custPhysicalAddByID($customer) { 
      $fpoid=$this->session->userdata('user_id');
	...
	function customerbranch_list($customer_id) {
		$fpoid=$this->session->userdata('user_id');
    ...
    function customerbranchByID($customer) { 
		  $fpoid=$this->session->userdata('user_id');
   ...
   function custBranchPhysicalAddByID($customer) { 
      $fpoid=$this->session->userdata('user_id');
      $this->db->select('trv_district_master.name As district_name,trv_state_master.name As state_name');
   ...
   function updatecustomerbranch($customer_branch_id) {
      $customer_branch_details = array(
         'fpo_id' 					=> $this->session->userdata('user_id'),
         'debtor_no' 				=> $this->input->post('customer'),
         'br_name' 				    => $this->input->post('branch_name'),
	...
	function branchdetail($debtor_no){  
      $fpoid=$this->session->userdata('user_id');	 
   ...
   function locationdetail(){
    ...
    function getAllRecurrentInvoices() {  
	...
	function getFpoDebtorTransLastTransNo() {
	...
	function add_salesentry() {		
      ...
      if($gst_enable == 1) {
         if($gst_type == 1) {
            $cust_gl_group = '101010103'; //Intra - Taxable
         }
         else if($gst_type == 2) {
            $cust_gl_group = '101010106'; //Inter - Taxable
         }
      }
      else {
         if($gst_type == 1) {
            $cust_gl_group = '101010101'; //Intra - Exempted
         }
         else if($gst_type == 2) {
            $cust_gl_group = '101010104'; //Inter - Exempted
         }
      }
...
//mobile number search//    
  function getMobileCustomers($search) {
	...
	function getFpoDebtorLastRefNo() {
   ...
   function getCustomerGL() {  
		  $fpoid=$this->session->userdata('user_id');
        $this->db->select('fpo_debtors_master.debtor_no, fpo_debtors_master.name, fpo_debtors_master.opening_balance, trv_chart_master.account_name');
        $this->db->from('fpo_debtors_master');
