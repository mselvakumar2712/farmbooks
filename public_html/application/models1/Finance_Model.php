...
/** Finance Transaction module post values starts **/    
	...
	function postBankAccountTransfer() {
...
/** April 10 **/ 
	...
	function closingBalanceGLcashreports($type, $fromValue, $toValue) {
	...
	function postGLPersonReports($type, $typeNo){
	...
	function getPersonGLReport($type, $person_id){
        $this->db->select('fpo_gl_trans.*, trv_chart_master.account_name, fpo_debtors_master.name As customerName, fpo_suppliers.supp_name As supplierName');
        if($this->input->post("amount_min") && $this->input->post("amount_min") != "0.00") {
            $this->db->where('fpo_gl_trans.amount >=', $this->input->post("amount_min"));
        }
    ...
    function postBankAccountInquiry() {
        $this->db->select('fpo_gl_trans.*, fpo_bank_accounts.bank_account_name, fpo_bank_accounts.bank_account_number');
	...
	//function getAllGLParentByCode($parent_code) {
		...
		//return $this->db->get()->result();	
	//} 
	function getAllGLParent($fpo_id, $fromValue, $toValue, $selectedCode, $account_mode) {
	...
	function getAllInventorySum($fpo_id, $fromValue, $toValue, $account_mode) {
		if($account_mode == 1) {
	...
	function getAllCashInBankAmount($fpo_id, $fromValue, $toValue, $account_mode=null) {
		$this->db->select('sum(fpo_gl_trans.amount) as total_amount');       
	...
	function getLedgerBalance($fpo_id, $fromValue, $toValue, $account_mode) {
		$this->db->select('sum(fpo_gl_trans.amount) as total_amount');       
		$this->db->where(array('fpo_gl_trans.fpo_id' => $fpo_id, 'fpo_gl_trans.status' => 1));	
		$this->db->where('fpo_gl_trans.tran_date =', $fromValue);
	...
	function getAllGLParentCost($fpo_id, $fromValue, $toValue, $selectedCode) {
		$selectedCodeLength = strlen($selectedCode);
	...
	function getAllLedgerByParents($fpo_id, $fromValue, $toValue, $selectedCode) {
		$selectedCodeLength = strlen($selectedCode);
	...
	function getAllChildNodeByCode($fpo_id){
		...
		if($this->input->post('person_type_id') == 1){
			...
			$this->db->join('fpo_suppliers', 'fpo_suppliers.supplier_id = fpo_gl_trans.account', 'left');
		} else if($this->input->post('person_type_id') == 2){
			$this->db->select('fpo_debtors_master.name as debtor_name');
			$this->db->join('fpo_debtors_master', 'fpo_debtors_master.debtor_no = fpo_gl_trans.account', 'left');
		} else if($this->input->post('person_type_id') == 3){
		} else {
...
/** Finance Maintenance module function start **/      
   ...
   function bankByViewID($bank_name_id) {		
		$this->db->select('fpo_bank_accounts.*, trv_bank_name_master.name, trv_bank_master.branch_name, trv_bank_master.address_local, trv_fpo.producer_organisation_name');        
		  $this->db->from('fpo_bank_accounts');
		  $this->db->join('trv_bank_name_master', 'trv_bank_name_master.id = fpo_bank_accounts.bank_name', 'left');
		  $this->db->join('trv_bank_master', 'trv_bank_master.id = fpo_bank_accounts.bank_address', 'left');
...
//28/12
   ...
   function cashAccountGLAll() {
	...
	function glAccountgroup_view($id) {
		  $this->db->select('trv_chart_master.*');        
		  $this->db->from('trv_chart_master');
    ...
    function updateglAccount_group($gl_acc_grp_id) {  
			$glaccountgroup_details = array(
				'account_name'        	    => $this->input->post('name'),
				'account_type'             => $this->input->post('trans_type')
			);	
	...
	function glacc_groupByID($gl_acc_grp_id) {
		$this->db->select('trv_chart_master.*');        
	...
	function updateglAccounts($gl_acc_id) {  
			$glaccountgroup_details = array(			
	...
	function getSuppnameByInvoiceno($invoice_no){ 
	...
	function getCustnameByInvoiceno($invoice_no){
	...
	function getPurchaseAmount($invoice){ 
	...
	function getSalesAmount($invoice) {  
   ...
   function salesinvoicelist(){
      ...
      $this->db->where(array('fpo_gl_trans.type' => 6, 'fpo_gl_trans.account_code LIKE' => '40202%', 'fpo_gl_trans.status' =>1, 'fpo_gl_trans.fpo_id' => $_SESSION['user_id']));
      $this->db->from('fpo_gl_trans');
      ...
      $this->db->order_by("counter", "asc");
      $this->db->where(array('fpo_gl_trans.type' => 6, 'fpo_gl_trans.account_code LIKE' => '40205%', 'fpo_gl_trans.status' =>1, 'fpo_gl_trans.fpo_id' => $_SESSION['user_id']));
      $this->db->from('fpo_gl_trans');
   ...
   function getSalesTransactionById($voucher_no,$inv_no,$customer){
   ...
   function purchaseregisterlist(){
	...
	function getInventoryTransactionById($type, $voucher_no, $inv_no, $category_person){
		if($type == 1){
	...
	function getPurchaseTransactionById($voucher_no,$inv_no,$supplier){
	...
	function getUOMNameByID($uom_id){ 
   ...
   /* function getProductCountByID($product_id){ 
	...
	function getLastTransactionByFPO($fpo_id, $type){
	...
	function getAllTransactionByVoucherNumber($voucher_no){
	...
	function closingBalanceByVoucherNumber($voucher_no, $type, $holder){
        $this->db->select('SUM(fpo_gl_trans.amount) As balance');
		if($type == 'ledger'){
