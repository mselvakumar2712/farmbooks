   ...
   /* function getBankAddressByBankName() {
	...
	function add_supplier() {		
        $supplier_details = array(
            ...
            'supp_account_no'			   => $this->input->post('our_customer_no'),
        ...
        if($this->input->post('bank_info') == 1){
         ...
         $payment_details = array(	
            ...
            'voucher_no'        => 'trv_PT_'.$last_inserted_supplier_id,
            'type'              => 9,
	...
	function salespersonDropdownList() {
      $fpoid=$this->session->userdata('user_id');
	...
	function supplierByID($supplier) {
      $fpoid=$this->session->userdata('user_id');
	...
	function get_purchase_supplier() {
	...
	function add_location_adjustment() {
	...
	function add_location_transfer() {
	...
	function priceBystockID($stock_id) {
	...
	function get_all_supplier_detail() {  
		$fpoid=$this->session->userdata('user_id');
        $this->db->select('fpo_suppliers.*');
		$this->db->where(array('fpo_suppliers.fpo_id' => $fpoid));
        $this->db->order_by("fpo_suppliers.supplier_id", "desc");
	...
	function get_supplierDetail($supplier_id) {  
        $this->db->select('fpo_suppliers.*');
	...
	function invoiceNumberExists( $invoicenumber ) {
        $invoice_count = $this->db->get_where('fpo_supp_trans', array('supp_reference' => $invoicenumber))->num_rows();
		return $invoice_count;
    }
    
   function supplierInvoiceNumberExists() {
      $supplierId = $this->input->post('supplierId');
      $invoiceNo = $this->input->post('invoiceNo');
      $invoice_count = $this->db->get_where('fpo_supp_trans', array('supp_reference' => $invoiceNo, 'supplier_id' => $supplierId))->num_rows();
		return $invoice_count;
   }
	
	function get_transfertype() {  
...
//mobile number search//  	
   function getMobileSuppliers($search) { 
   ...
   function getSupplierGL() {  
		$fpoid=$this->session->userdata('user_id');
        $this->db->select('fpo_suppliers.supplier_id, fpo_suppliers.supp_name, fpo_suppliers.opening_balance, trv_chart_master.account_name');
        $this->db->from('fpo_suppliers');
   ...
   function get_mrplist($product_id){ 
      $fpoid=$this->session->userdata('user_id');   
   ...
   function get_SellingPricelist($price_id){
