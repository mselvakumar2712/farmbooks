...
/** Share Application with FPO related functions start here **/    
   ...
   /** Share Application Allotment Starts **/ 
		...
		//if($share_released <= $total_avaialable_share->shares_available){
			for($i=0; $i<count($share_application); $i++){
				if($share_application[$i]['share_alloted'] != 0 && $share_application[$i]['share_alloted'] > 0){
					...
					$total_share=0;$new_share=0;$existing_share=0;
					if($shareAllotted){						
    ...
    function getLocationFpo($fpo_id) {
	...
	function getAllotmentByID($folio_number, $holder_id, $member_type, $type){ 
    ...
    function getShareAlottmentID($fpo_id){
   ...
   function generateApplicationList() {
      $this->db->select('trv_share_application.*, trv_farmer.profile_name, trv_fpo.producer_organisation_name');	
	 ...
	 function generateApplication($shares_id) {
  ...
  function fpoByUser_ID($fpo_id) { 
	...
	function shareBonusAppliedListByCutOffDate($cut_off_date) {
    ...
    function shareRightAppliedListByCutOffDate($cut_off_date) {
    ...
    function listAllocatedMember(){		
	 ...
	 function getAllocatedByID() {
		...
		}else if(!empty($resolution_number)){
			$this->db->select('trv_share_history.*');
			$this->db->from('trv_share_history');
	 ...
	 function postOriginalCertificate() {
	...
	function getAllocatedBonusByID() {
		$resolution_date=$this->input->post('resolution_date');
		$resolution_number=$this->input->post('resolution_number');
		if(!empty($resolution_date and $resolution_number)){
			$this->db->select('trv_share_history.*');
	...
	function getAllocatedRightByID() {
		$resolution_date=$this->input->post('resolution_date');
		$resolution_number=$this->input->post('resolution_number');
		if(!empty($resolution_date and $resolution_number)){
			$this->db->select('trv_share_history.*');
	...
	function get_certificate_last_id() {
    ...
    function get_bonuscertificate_last_id() {
	...
	function get_rightcertificate_last_id() {
    ...
    function verifyMember($member_id, $member_type) {
		$this->db->select('trv_share_allotment.*');
		$this->db->from('trv_share_allotment');
    ...
    function getShareByMember($holder_id, $member_type) {
	...
	function verifyRegisteredNumber(){
      if(!empty($this->input->post('mobilenumber'))){
   ...
   function viewShareDetails($holder_id, $member_type, $folio_number) {
       $data_farmer = $this->get_farmer_list();
       $data_fpo = $this->get_fpo_list();
...
/** Share holders folio number **/
	function getShareHolersFolioNo($holder_id, $member_type){
	...
	function getShareHoldersFolioCount($fpo_id, $holder_id=null, $member_type=null){	
	...
	function getShareHolersByFolioNo($folio_no, $holder_id, $member_type) {
   ...
   function fpofarmerpanchayatList() {
...
/** Share settings Module */
            ...
            if($this->input->post('paid_capital') ==1)
               {
                 ...
                 if($sharebycash){
                    $shareamt = $sharebycash - $sharebycash;
                 }else{
                    $shareamt = $sharetotal;
                 }
   ...
   function postfpo_farmer() {
    $farmer_id =$this->input->post('farmer_id');
    $shares_held =$this->input->post('shares_held');
    $folio_number = $this->input->post('folio_number');  
    for($i=0;$i<count($farmer_id);$i++){ 
        $getid=$this->db->select('trv_share_allotment.id')->where(array('trv_share_allotment.holder_id' =>$farmer_id[$i],'trv_share_allotment.fpo_id' =>$this->session->userdata('user_id')))->from('trv_share_allotment')->get()->row()->id;
      $add_fpo_farmer = array(
         'fpo_id'               => $this->session->userdata('user_id'),
         'allotment_nature'     => 10001,
