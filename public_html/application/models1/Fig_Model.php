...
class Fig_Model extends CI_Model {
   ...
   function figList() {
    ...
    function addfig() {
      ...
      $created_fig_user_id = '4'.$rest.str_pad(($total_field+1), 3,'0', STR_PAD_LEFT);
   ...
   /*  function figByID($fig_id) {
     ...
     function figByID($fig_id) {
		 $this->db->select('trv_fig.*,trv_state_master.name As state_name,trv_district_master.name As district_name,trv_taluk_master.name As taluk_name,trv_block_master.name As block_name,trv_village_master.name As village_name,trv_panchayat_master.name As panchayat_name');
		...
		$this->db->join('trv_taluk_master', 'trv_taluk_master.taluk_code = trv_fig.taluk');
		$this->db->join('trv_block_master', 'trv_block_master.block_code = trv_fig.block'); */
		$this->db->where(array('trv_fig.id' => $fig_id)); $this->db->from('trv_fig');
      return $this->db->get()->row_array();
   }
...
/** FIG Representative Starts **/
    function figrepresentList() {
        ...
        $this->db->where(array('trv_fig_representative.status' => 1,'trv_fig.fpo_id' => $this->session->userdata('user_id')));
        return $this->db->get()->result();		
    }
...
/** FIG Representative End **/           
	...
	function mobileNumberExists( $mobile) {
        $mobile_number = $this->db->get_where('trv_farmer', array('mobile' => $mobile))->num_rows();
		//$mobile_number = $this->db->or_where('trv_farmer',array('aadhar_no' => $aadhar_no, 'mobile' => $mobile))->num_rows();
	...
	function fig_list() {
        ...
        $this->db->from('trv_fig');
		$this->db->distinct();
        $this->db->where(array('trv_fig.fpo_id' =>  $this->session->userdata('user_id'), 'trv_fig.status' => '1'));
        return $this->db->get()->result();
    }
