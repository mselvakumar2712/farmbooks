...
class Login_Model extends CI_Model {
  ...
  function fpgCount($fpo_id) {
   ...
   function salespurchasemonth($fpo_id) {
      ...
      $this->db->select('fpo_gl_trans.counter,MONTHNAME(fpo_gl_trans.tran_date) as month');
      $this->db->from('fpo_gl_trans');
      $this->db->where(array('fpo_gl_trans.fpo_id' => $fpo_id, 'fpo_gl_trans.status' => 1));
      $this->db->order_by("month(fpo_gl_trans.tran_date)", "ASC");
      $this->db->group_by("fpo_gl_trans.tran_date");       
      return $this->db->get()->result();	
   } 
   ...
   public function count_purchase($fpo_id){
      $this->db->select('fpo_gl_trans.type,fpo_gl_trans.amount');
      $this->db->from('fpo_gl_trans');
   ...
   public function count_sales($fpo_id){
      $this->db->select('fpo_gl_trans.type,fpo_gl_trans.tran_date,MONTHNAME(fpo_gl_trans.tran_date) as month,YEAR(fpo_gl_trans.tran_date) as year');
      $this->db->from('fpo_gl_trans');
      $this->db->where(array('fpo_gl_trans.fpo_id' => $fpo_id, 'fpo_gl_trans.status' => 1,'fpo_gl_trans.type' => 6));
      $this->db->group_by('YEAR(fpo_gl_trans.tran_date),month(fpo_gl_trans.tran_date)');
      $this->db->limit(10);
      return $this->db->get()->result_array();	
   } 
